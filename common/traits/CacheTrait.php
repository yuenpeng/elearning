...
trait CacheTrait
{
     ...
     */
    public static function saveToCache($cacheKey, $cacheValue, $dependencySql = null, $duration = 3600, $withCache = true, $unsetUnNeedInfo = false)
    {
        if ($withCache) {
            if ($unsetUnNeedInfo && isset($cacheValue->oldAttributes)) {
                self::unsetUnNeedInfo($cacheValue);
            }
...
//                $bool = Yii::$app->cache->exists($cacheKey);
            ...
            //保存缓存后，需要恢复现场（因为有可能当场会再使用这个数据进行更新操作）
            if ($unsetUnNeedInfo && isset($cacheValue->attributes)) {
                self::loadUnNeedInfo($cacheValue);
            }
        }
    }

    public static function unsetUnNeedInfo($model) {
        if ($model != null) {
            unset($model->oldAttributes);
        }
    }

    public static function loadUnNeedInfo($model) {
        if ($model != null) {
     ...
     * @return mixed|null
     */
    public static function loadFromCache($cacheKey, $withCache = true, &$hasCache = false, $loadUnNeedInfo = false) {
        $result = null;
        $hasCache = false;
        if ($withCache) {
            if (Yii::$app->cache->exists($cacheKey)) {
                ...
                if ($loadUnNeedInfo && isset($result->attributes)) {
                    self::loadUnNeedInfo($result);
                }
            }
        }

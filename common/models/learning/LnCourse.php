...
class LnCourse extends BaseActiveRecord
{
	...
	//收费课程类型
	const STUDY_TYPE_DEFAULT = '0';
	const STUDY_TYPE_BX = '1';//必修
	const STUDY_TYPE_XX = '2';//选修
    ...
    public function rules()
    {
        return [
			...
			[['study_type'], 'in', 'range' => [self::STUDY_TYPE_DEFAULT, self::STUDY_TYPE_BX, self::STUDY_TYPE_XX]],
            [['study_type'], 'default', 'value'=> self::STUDY_TYPE_DEFAULT],
    ...
    public function attributeLabels()
    {
        return [
			...
			'is_charge_approval' => Yii::t('common', 'is_charge_approval'),
			'study_type' => Yii::t('common', 'study_type'),
    ...
    */
    ...
    public function getCourseCollectionText()
    {
        ...
        if (!empty($data) && $data > 0) {
            return Yii::t('common', 'cancel_collection');
        } else {
    ...
    public function getLnCourseReg()
    {
        return $this->hasOne(LnCourseReg::className(), ['course_id' => 'kid'])
            ->onCondition([LnCourseReg::realTableName() . '.is_deleted' => self::DELETE_FLAG_NO]);
    }
}

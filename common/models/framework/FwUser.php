...
use common\helpers\TLadpHelper;
 ...
 * @property string $online_duration
...
class FwUser extends BaseActiveRecord implements IdentityInterface
{
	...
	const TO_EMAIL_YES = "1";
    const TO_EMAIL_NO  = "0";
	
	const TO_MOBILE_NO_YES = "1";
    const TO_MOBILE_NO_NO  = "0";
	
	const IS_SPARE_YES = "1";
    const IS_SPARE_NO  = "0";
...
//            [['user_name', 'is_deleted'], 'unique', 'on' => 'api-manage-add','message'=>Yii::t('common','unique_constraint_error')],
            ...
            [['user_name','real_name', 'nick_name', 'kid','id_number','location', 'reporting_manager_id', 'company_id', 'orgnization_id',
                'domain_id', 'cost_center_id', 'data_from',
                'created_by', 'updated_by','language','timezone'], 'string', 'max' => 50],
            [['auth_token', 'auth_key', 'password_reset_token', 'account_active_token', 'email', 'find_pwd_tmp_key'], 'string', 'max' => 255],
...
//            [['password_origin'], 'string', 'min'=>6, 'max' => 255],
            ...
            [['online_duration'], 'default', 'value' => 0],
			...
			[['to_email'], 'string', 'max' => 1],
            [['to_email'], 'in', 'range' => [self::TO_EMAIL_NO, self::TO_EMAIL_YES]],
			
			[['to_mobile_no'], 'string', 'max' => 1],
            [['to_mobile_no'], 'in', 'range' => [self::TO_MOBILE_NO_NO, self::TO_MOBILE_NO_YES]],
			
			[['is_spare'], 'string', 'max' => 1],
            [['is_spare'], 'in', 'range' => [self::IS_SPARE_NO, self::IS_SPARE_YES]],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
			...
			'to_email' => Yii::t('common', 'to_email'),
			'email2' => Yii::t('common', 'email2'),
			'to_mobile_no' => Yii::t('common', 'to_mobile_no'),
			'mobile_no2' => Yii::t('common', 'mobile_no2'),
...
//            'password_origin' => Yii::t('common', 'password_origin'),
    ...
    public function getDisplayName() {
        if (!empty($this->email)) {
            return $this->real_name . "(" . $this->email . ")".(!empty($this->usser_no)?'_'.$this->user_no:'');
        }
        else {
            return $this->real_name  . "(" . $this->user_name . ")".(!empty($this->usser_no)?'_'.$this->user_no:'');
        }
    }
     ...
     */
    public function validatePassword($password,$flag = null)
    {
    	$validatePasswordResult=false;
    	if('ladp'==$flag){
    		$username=$this->user_name;
    		$tLadpHelper=new TLadpHelper();
    		$validatePasswordResult=$tLadpHelper->validatePawdbyLadp($username, $password);
    	}else{
    		$validatePasswordResult=Yii::$app->security->validatePassword($password, $this->password_hash);
    	}  	
    ...
    public function getList($params = array(), $debug = 0) {
        ...
        if (empty($params['select']) && empty($params['field'])) {
            $params['select'] = 'kid,real_name,user_no,email,to_email,email2,mobile_no,to_mobile_no,mobile_no2,is_spare'
                    . ',created_at,created_by,updated_by,updated_at';
        }
        $sult = BoeBaseActiveRecord::getList($params);
        $tmp_arr = NULL;
	...
	public function saveInfo($data) {
        $kid	= $data['kid'];
		unset($data['kid']);
		return $this->updateAll($data, " kid = '{$kid}'");
    }

 ...
 */
class BoeResourceFileModel extends BoeBaseActiveRecord {

    /**
     * @inheritdoc
     */
    public static function tableName() {
        return 'eln_boe_resource_file';
    }
...
//-----------------------1大堆和写数据库有关的方法开始------------------------------------------- 
     ...
     * @return boolean
     */
    public function updateResourceId($kid = '', $resource_id = '', $debug = 0) {
        $currentObj = $this->findOne([$this->tablePrimaryKey => $kid]);
        if ($currentObj) {
            $md5_key = $currentObj->md5_key;
            $currentObj->resource_id = $resource_id;
            ...
            if ($opreateSult) {//操作成功
                ...
                $error = $this->getErrors();
                if ($debug) {
                    $text = ("更新ID是{$kid}的文件的ResourceID值失败!\n");
                    $text.=("错误:\n");
                    $text.=($error);
                    BoeBase::debug($text, 1);
                }

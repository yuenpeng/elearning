...
/**
 * This is the model class for table "eln_boe_doc_share_user".
 *
 * @property string $kid
 ...
 */
class BoeDocShareUser extends BoeBaseActiveRecord {

    /**
     * @inheritdoc
     */
    public static function tableName() {
        return 'eln_boe_doc_share_user';
    }
...
//            [['kid', 'created_by', 'created_at'], 'required'],
     ...
     * @return type
     */
    public function getUserShareDocInfo($user_id = 0, $create_mode = 0,$debug=false) {
        if (!$user_id) {
            return NULL;
        }
        ...
        if ($create_mode == 2) {//删除缓存模式时S
            ...
            if (!$cacheSult) {//缓存中没有数据的时候S
                ...
                $cacheSult = $this->find(false)->select('user_id,doc_id')->where($where_p)->indexby('doc_id')->asArray()->all();
                if ($cacheSult) {
     ...
     * @return type
     */
    public function getDocShareUserInfo($id = 0, $return_name = false, $create_mode = 0,$debug=false) {
        if (!$id) {
            return NULL;
        }
        ...
        if ($create_mode == 2) {//删除缓存模式时S
            ...
            if (!$cacheSult) {//缓存中没有数据的时候S
                ...
                $where_p[] = ['doc_id' => $id];
                $where_p[] = ['is_deleted' => 0];
                $cacheSult = $this->find(false)->select('user_id,doc_id')->where($where_p)->indexby('user_id')->asArray()->all();
                if ($cacheSult) {
                    $this->setCache(__METHOD__, $cacheSult, 0, $debug); // 设置缓存
                }
            }//缓存中没有数据的时候S  
    ...
    /**
     * 读取某个文档对应的共享用户信息的@字符串
     * @param type $doc_id
     * @return type
     */
    public function getOneDocShareUserAtString($doc_id) {
        return  $this->getDocShareUserInfo($doc_id, 1);

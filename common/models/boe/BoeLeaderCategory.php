 ...
 * @property integer $course_category_code
...
class BoeLeaderCategory extends BoeBaseActiveRecord {
    ...
    public static function tableName() {
        return 'eln_boe_leader_category';
    }
...
//            [['kid', 'name', 'created_by', 'created_at'], 'required'],
            [['sort', 'version', 'created_at', 'updated_at'], 'integer'],
            [['kid', 'name', 'course_category_code','parent_id', 'created_by', 'created_from', 'created_ip', 'updated_by', 'updated_from', 'updated_ip'], 'string', 'max' => 50],
...
//                    $sort_info['index_sort'][$key] = $a_info['index_sort'];
    ...
    public function saveInfo($data, $debug = 0) {
        ...
        if (!empty($data[$this->tablePrimaryKey])) {//修改的时候  
            ...
            $allSubInfo = $this->getSubId($currnetKid, 'kid'); //找到子分类
            if (in_array($data['parent_id'], $allSubInfo)) {
                $opreateSult = false;
                $currnetKid = false;
            } else {
    ...
    public function getCotesByTags($tag){
        $obj = $this->find(false)
            ->andFilterWhere(['is_deleted'=>0]);
        if($tag == 'all'){
            return $obj->asArray()->all();
        }else{
            return $obj->andFilterWhere(['list_tag'=>$tag])->asArray()->all();
        }

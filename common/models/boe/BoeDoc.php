 ...
 */
class BoeDoc extends BoeBaseActiveRecord {
    ...
    public function rules() {
        return [
            ...
            //[[ 'reprot_num','visit_num',   'share_num'], 'integer'],//这几个字段是用mysql函数来完成的
            [['kid', 'user_id', 'file_id', 'keyword1', 'keyword2', 'keyword3', 'keyword4', 'keyword5', 'keyword6', 'keyword7', 'keyword8', 'keyword9', 'keyword10', 'created_by', 'created_from', 'created_ip', 'updated_by', 'updated_from', 'updated_ip'], 'string', 'max' => 50],
    ...
    public function attributeLabels() {
        return [
            'kid' => Yii::t('boe', 'doc_kid'),
            'title' => Yii::t('boe', 'doc_title'),
            'user_id' => 'User ID',
            'file_id' => 'File ID',
            'category_id' => Yii::t('boe', 'doc_category_id'),
            'abstract' => Yii::t('boe', 'doc_abstract'),
            'image_url' => Yii::t('boe', 'doc_image_url'),
            'has_image' => 'Has Image',
            'recommend_sort1' => Yii::t('boe', 'doc_index_sort'),
            ...
            'down_num' => 'Down Num',
            'visit_num' => Yii::t('boe', 'doc_visit_num'),
    ...
    public function getList($params = array(), $debug = 0) {
        ...
        if (empty($params['select']) && empty($params['field'])) {
            $params['select'] = 'kid,title,user_id,file_id,category_id,abstract,image_url,has_image,'
    ...
    public function updateShareNum($kid = '', $debug = 0) {
        ...
        if ($currentObj) {
            $currentObj->share_num = new Expression("get_doc_share_num('{$kid}')");
            $opreateSult = $currentObj->save();
            if ($opreateSult) {//操作成功
                return true;
            } else {//操作失败
    ...
    public function updateReportNum($kid = '', $debug = 0) {
        $currentObj = $this->findOne([$this->tablePrimaryKey => $kid]);
        if ($currentObj) {
            $currentObj->reprot_num = new Expression("get_doc_report_num('{$kid}')");
            $opreateSult = $currentObj->save();
            if ($opreateSult) {//操作成功
                return true;
            } else {//操作失败

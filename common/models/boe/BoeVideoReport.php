...
/**
 * This is the model class for table "eln_boe_video_report".
 *
 * @property string $kid
 ...
 */
class BoeVideoReport extends BoeBaseActiveRecord {

    /**
     * @inheritdoc
     */
    public static function tableName() {
        return 'eln_boe_video_report';
    }
...
//            [['kid', 'created_by', 'created_at'], 'required'],
     ...
     * @return type
     */
    public function getVideoReport($id = 0, $return_name = false, $create_mode = 0) {
        if (!$id) {
            return NULL;
        }
        ...
        if ($create_mode == 2) {//删除缓存模式时S
            ...
            if (!$cacheSult) {//缓存中没有数据的时候S
                ...
                $cacheSult = $this->find(false)->select('kid,video_id,user_id,content,created_by,created_at')->where($where_p)->orderBy('created_at desc')->asArray()->all();
                if ($cacheSult) {
    ...
    public function addReport($data, $debug = 0) {
        ...
        if ($opreateSult) {//操作成功
            if (!$currnetKid) {//添加的时候
                ...
                $this->getVideoReport($currnetKid, flase, 2); //更新缓存
            }
        } else {//操作失败
            if ($debug) {

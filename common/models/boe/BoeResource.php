 ...
 */
class BoeResource extends BoeBaseActiveRecord {
    ...
    public function attributeLabels() {
        return [
            'kid' => Yii::t('boe', 'resource_kid'),
            'title' => Yii::t('boe', 'resource_title'),
            'user_id' => 'User ID',
            'file_id' => 'File ID',
            'category_id' => Yii::t('boe', 'resource_category_id'),
			'abstract' => Yii::t('boe', 'resource_abstract'),
            'audience_id' => Yii::t('boe', 'audience_id'),
            'image_url' => Yii::t('boe', 'resource_image_url'),
            'has_image' => 'Has Image',
            'recommend_sort1' => Yii::t('boe', 'resource_index_sort'),
            ...
            'down_num' => 'Down Num',
            'visit_num' => Yii::t('boe', 'resource_visit_num'),
    ...
    public function getList($params = array(), $debug = 0) {
        ...
        if (empty($params['select']) && empty($params['field'])) {
            $params['select'] = 'kid,title,user_id,file_id,category_id,audience_id,abstract,image_url,has_image,'
    ...
    public function updateShareNum($kid = '', $debug = 0) {
        ...
        if ($currentObj) {
            $currentObj->share_num = new Expression("get_resource_share_num('{$kid}')");
            $opreateSult = $currentObj->save();
            if ($opreateSult) {//操作成功
                return true;
            } else {//操作失败
    ...
    public function updateReportNum($kid = '', $debug = 0) {
        $currentObj = $this->findOne([$this->tablePrimaryKey => $kid]);
        if ($currentObj) {
            $currentObj->reprot_num = new Expression("get_resource_report_num('{$kid}')");
            $opreateSult = $currentObj->save();
            if ($opreateSult) {//操作成功
                return true;
            } else {//操作失败

...
/**
 * This is the model class for table "eln_boe_resource_share_user".
 *
 * @property string $kid
 ...
 */
class BoeResourceShareUser extends BoeBaseActiveRecord {

    /**
     * @inheritdoc
     */
    public static function tableName() {
        return 'eln_boe_resource_share_user';
    }
...
//            [['kid', 'created_by', 'created_at'], 'required'],
            ...
            [['kid', 'resource_id', 'user_id', 'created_by', 'created_from', 'created_ip', 'updated_by', 'updated_from', 'updated_ip'], 'string', 'max' => 50],
     ...
     * @return type
     */
    public function getUserShareResourceInfo($user_id = 0, $create_mode = 0,$debug=false) {
        if (!$user_id) {
            return NULL;
        }
        ...
        if ($create_mode == 2) {//删除缓存模式时S
            ...
            if (!$cacheSult) {//缓存中没有数据的时候S
                ...
                $cacheSult = $this->find(false)->select('user_id,resource_id')->where($where_p)->indexby('resource_id')->asArray()->all();
                if ($cacheSult) {
     ...
     * @return type
     */
    public function getResourceShareUserInfo($id = 0, $return_name = false, $create_mode = 0,$debug=false) {
        if (!$id) {
            return NULL;
        }
        ...
        if ($create_mode == 2) {//删除缓存模式时S
            ...
            if (!$cacheSult) {//缓存中没有数据的时候S
                $where_p = array('and');
                $where_p[] = ['resource_id' => $id];
                $where_p[] = ['is_deleted' => 0];
                $cacheSult = $this->find(false)->select('user_id,resource_id')->where($where_p)->indexby('user_id')->asArray()->all();
                if ($cacheSult) {
                    $this->setCache(__METHOD__, $cacheSult, 0, $debug); // 设置缓存
                }
            }//缓存中没有数据的时候S  
     ...
     * @return type
     */
    public function getOneResourceShareUserAtString($resource_id) {
        return  $this->getResourceShareUserInfo($resource_id, 1);
     ...
     * @return boolean
     */
    public function updateResourceShareUserInfo($resource_id = 0, $user_info = array()) {
        if (!$resource_id) {
            return false;
        }
        $this->physicalDeleteAll(array('resource_id' => $resource_id)); //删除之前的记录
        ...
        $this->getResourceShareUserInfo($resource_id, false, 2); //删除视频对应的用户关系缓存数据
        foreach ($user_id as $a_user_id) {
            $this->getUserShareResourceInfo($a_user_id, 2); //删除用户对应视频的关系缓存数据
        }
        return true;
    }

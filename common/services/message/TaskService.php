...
//                    $t['count'] = count($user);
    ...
    public function getTaskObjectByTaskId($task_id, $key, $size, $is_fail = false)
    {
        ...
        if ($key != null && $key != '') {
            $query->andFilterWhere(['or', ['like', 'u.real_name', $key], ['like', 'u.user_no', $key]]);
        }
        ...
        foreach ($items as $item) {
            if ($item->item_type === MsTaskItem::ITEM_TYPE_COURSE) {
                ...
                $courseQuery
                    ...
                    ->andFilterWhere(['=', LnCourse::tableName() . '.kid', $item->item_id])
                    ->select([LnCourse::tableName() . '.*', 'audienceName' => 'GROUP_CONCAT(au.audience_name)'])
                    ->groupBy(LnCourse::tableName() . '.kid');

                $courseModel=$courseQuery->one();
                if($courseModel && $courseModel->audienceName)
                {
                    $auFlag='2';
                }
                else{

...
//                    }
     ...
     */
    public function get_scorm_default($scormVersion, &$userdata, $courseRegId,$modResId, $scoId,  $attempt, $mode, $withSession = false) {
        ...
        if (!empty($user)) {
            ...
            $userdata->student_name = $user->real_name;
        } else {
            $userdata->student_id = null;
            $userdata->student_name = "匿名";
        }
        ...
        if (isset($userdata->status)) {
            if ($userdata->status == '') {
                ...
                $exitTag = "cmi.core.exit";
                if ($this->scorm_version_check($scormVersion, self::SCORM_13)) {
                    $exitTag = "cmi.exit";
                }
                if (isset($userdata->{$exitTag}) && ($userdata->{$exitTag} == 'suspend')) {
        ...
        $def = array();
        $tempMasteryScore = null;
        $masteryScoreElement = 'cmi.student_data.mastery_score';
        if ($this->scorm_version_check($scormVersion, self::SCORM_13)) {
            $masteryScoreElement = "cmi.mastery_score";
        }
        if (!empty($userdata->$masteryScoreElement)) {
            $tempMasteryScore = $userdata->$masteryScoreElement;
        }
        //上传课件的时候，已经统一转换成mastery_score
//        else if (!empty($userData->masteryscore)) {
//            $tempMasteryScore = $userData->masteryscore;
//        }
        else if (!empty($userdata->mastery_score)) {
            $tempMasteryScore = $userdata->mastery_score;
        }
        //2004 only
        if ($this->scorm_version_check($scormVersion, self::SCORM_13)) {
            $def['cmi.learner_id'] = $userdata->student_id;
            $def['cmi.learner_name'] = $userdata->student_name;
            $def['cmi.location'] = $this->scorm_isset($userdata->{'cmi.location'});//记录上次学习位置
            $def['cmi.score.raw'] = $this->scorm_isset($userdata->{'cmi.score.raw'});//最后一次尝试的成绩
            $def['cmi.score.scaled'] = $this->scorm_isset($userdata->{'cmi.score.scaled'});//成绩权重,range 0.0-1.0
            $def['cmi.score.max'] = $this->scorm_isset($userdata->{'cmi.score.max'});//最高成绩
            $def['cmi.score.min'] = $this->scorm_isset($userdata->{'cmi.score.min'});//最低成绩
            $def['cmi.exit'] = $this->scorm_isset($userdata->{'cmi.exit'});
            $def['cmi.completion_status'] = $this->scorm_isset($userdata->{'cmi.completion_status'});//课程完成状态，有“completed”, “incomplete”,“not attempted”, “unknown”
            $def['cmi.success_status'] = $this->scorm_isset($userdata->{'cmi.success_status'});//是否合格，有“passed”, “failed”, “unknown”
            $def['cmi.credit'] = $userdata->credit;//学分，有credit和no-credit可选
            $def['cmi.entry'] = $userdata->entry;//进入方式
            $def['cmi.mode'] = $userdata->mode;//学习模式
            $def['cmi.total_time'] = $this->scorm_isset($userdata->{'cmi.total_time'}, '00:00:00');//总共学时时长

            $def['cmi.mastery_score'] = $this->scorm_isset($tempMasteryScore);//掌握所必须的成绩
            $def['cmi.max_time_allowed'] = $this->scorm_isset($userdata->maxtimeallowed);//最大可学时长
            $def['cmi.time_limit_action'] = $this->scorm_isset($userdata->timelimitaction);//超过最大学习时长后采取的动作

            $def['cmi.learner_preference.language'] = $this->scorm_isset($userdata->{'cmi.learner_preference.language'});
            $def['cmi.learner_preference.audio'] = $this->scorm_isset($userdata->{'cmi.learner_preference.audio'}, '0');
            $def['cmi.learner_preference.speed'] = $this->scorm_isset($userdata->{'cmi.learner_preference.speed'}, '0');
            $def['cmi.learner_preference.text'] = $this->scorm_isset($userdata->{'cmi.learner_preference.text'}, '0');

            $def['cmi.progress_measure'] = $this->scorm_isset($userdata->{'cmi.progress_measure'});
            $def['cmi.scaled_passing_score'] = $this->scorm_isset($userdata->{'cmi.scaled_passing_score'});

            $def['adl.browser_version'] = $_SERVER['HTTP_USER_AGENT'];
        }
        else {
            ...
            $def['cmi.core.lesson_location'] = $this->scorm_isset($userdata->{'cmi.core.lesson_location'});//记录上次学习位置
            $def['cmi.core.score.raw'] = $this->scorm_isset($userdata->{'cmi.core.score.raw'});//最后一次尝试的成绩
            $def['cmi.core.score.max'] = $this->scorm_isset($userdata->{'cmi.core.score.max'});//最高成绩
            $def['cmi.core.score.min'] = $this->scorm_isset($userdata->{'cmi.core.score.min'});//最低成绩
            $def['cmi.core.exit'] = $this->scorm_isset($userdata->{'cmi.core.exit'});
            $def['cmi.core.lesson_status'] = $this->scorm_isset($userdata->{'cmi.core.lesson_status'});//课程完成状态，有passed, completed, failed, incomplete, browsed, not attempted可选
            $def['cmi.core.credit'] = $userdata->credit;//学分，有credit和no-credit可选
            $def['cmi.core.entry'] = $userdata->entry;//进入方式
            $def['cmi.core.lesson_mode'] = $userdata->mode;//学习模式
            $def['cmi.core.total_time'] = $this->scorm_isset($userdata->{'cmi.core.total_time'}, '00:00:00');//总共学时时长

            $def['cmi.student_data.mastery_score'] = $this->scorm_isset($tempMasteryScore);//掌握所必须的成绩
            $def['cmi.student_data.max_time_allowed'] = $this->scorm_isset($userdata->maxtimeallowed);//最大可学时长
            $def['cmi.student_data.time_limit_action'] = $this->scorm_isset($userdata->timelimitaction);//超过最大学习时长后采取的动作
     ...
     */
    private function scorm_isset($param, $ifEmpty = '') {
        if (!is_null($param)) {
            return $param;
        } else {
            return $ifEmpty;

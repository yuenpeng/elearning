...
//                $condition = 'course_reg_id = :course_reg_id and mod_res_id = :mod_res_id AND attempt < :attempt and attempt > 1';
    ...
    public function getScoesTrackResultByAttempt($courseRegId, $modResId, $scoId, $attempt, $withSession = false)
    {
        if (!empty($courseRegId)) {
            ...
            if ($withSession && Yii::$app->cache->exists($sessionKey)) {
                $finalResult = Yii::$app->cache->get($sessionKey);
            } else {
                if (!$isUseMongoDB) {
                    $model = new LnScormScoesTrack();
...
//            ->addOrderBy(['element' => SORT_ASC])
                ...
                if ($withSession && Yii::$app->cache->exists($alreadySessionedListKey)) {
                    $alreadySessionedList = Yii::$app->cache->get($alreadySessionedListKey);
                }
                ...
                if (!empty($result)) {
                    if (!$isUseMongoDB) {
                        foreach ($result as $track) {
                            ...
                            if ($withSession) {
                                Yii::$app->cache->add($sessionElementKey, $track);
                        ...
                        if (isset($result->elementlist)) {
                            foreach ($result->elementlist as $element => $value) {
                                ...
                                if ($withSession) {
                                    Yii::$app->cache->set($sessionElementKey, $temp);
                                    $this->addSessionKey($sessionElementKey);
                    ...
                    if ($withSession) {
                        Yii::$app->cache->set($alreadySessionedListKey, $alreadySessionedList);
                        $this->addSessionKey($alreadySessionedListKey);
                    }
                }

                if ($withSession) {
                    Yii::$app->cache->set($sessionKey, $finalResult);
...
//            ->addOrderBy(['attempt' => SORT_ASC])
    ...
    public function getScormScoesTrackElementInfoByAttempt($courseRegId, $modResId, $scormScoId, $element, $attempt, $withSession = false)
    {
        if (!empty($courseRegId)) {
            ...
            if ($withSession && Yii::$app->cache->exists($sessionKey)) {
                $result = Yii::$app->cache->get($sessionKey);
            } else {
                if (!TBaseHelper::isUseMongoDB()) {
                    $model = new LnScormScoesTrack();
                    $result = $model->find(false)
...
//                        ->andFilterWhere(['=', 'element', $element])
                ...
                if ($withSession) {
                    $alreadySessionedList = [];
                    if (Yii::$app->cache->exists($alreadySessionedListKey)) {
                        $alreadySessionedList = Yii::$app->cache->get($alreadySessionedListKey);
                    }
    ...
    public function removeTrackSessionData($courseRegId, $modResId, $scoId, $attempt, $withSession = true)
    {
        if ($withSession) {
            ...
            if (Yii::$app->cache->exists($listSessionKey)) {
                Yii::$app->cache->delete($listSessionKey);
            }
            ...
            if (Yii::$app->cache->exists($alreadySessionedListKey)) {
                ...
                if (count($alreadySessionedList) > 0) {
                    ...
                    foreach ($alreadySessionedList as $elementKey) {
                        if (Yii::$app->cache->exists($elementKey)) {
                            Yii::$app->cache->delete($elementKey);
                        }
                    }

                    Yii::$app->cache->delete($alreadySessionedListKey);
                }
            }
        }
    }
    ...
    public function checkIsScormScoesCompletedByAttempt($courseRegId, $modResId, $scormScoId, $attempt, $scorm, $withSession = false)
    {
        ...
        if ($scorm->scorm_version == self::SCORM_13) {
            ...
            if (in_array($result->value, ['completed', "passed"])) {
                //g.如果completed，则认为是合格。这种情况课程算是全部完成。
                //h.如果passed，则认为合格。这种情况课程算是部分完成，课程有可能继续再学。
                return true;
            } else {
                //i.否则，则认为不合格
                return false;
                                    ...
                                    $trackdata = null, $withSession = false, $forcecompleted = false, $systemKey = null, &$courseComplete = false, &$getCetification = false, $isRepeatUpdate = true)
    {
        if (!empty($courseRegId) && !empty($courseCompleteProcessId) && !empty($courseCompleteFinalId)) {
            ...
            $elementSessionKey = "ScoesTrackElementResultAttempt_CourseRegId_" . $courseRegId . "_ModResId_" . $modResId . "_ScoId_" . $scormScoId . "_Element_" . $element . "_Attempt_" . strval($attempt);
            if ($trackdata !== null) {
                if ($withSession && Yii::$app->cache->exists($listSessionKey)) {
                    $trackdata = Yii::$app->cache->get($listSessionKey);
                }
            }
            if ($trackdata !== null) {
...
//                                array_push($batchUpdateModel, $track);
            ...
            if ($withSession && $isChange) {
                Yii::$app->cache->set($elementSessionKey, $track);
                $this->addSessionKey($elementSessionKey);
                if ($trackdata !== null) {
                    Yii::$app->cache->set($listSessionKey, $trackdata);
...
//                                array_push($batchUpdateModel, $track);
                    ...
                    if ($withSession && $isSingleChange) {
                        Yii::$app->cache->set($elmentSessionKey, $track);
    ...
    {
        $keyList = Yii::$app->cache->get(self::SESSION_KEY_LIST) ?: [];
        array_push($keyList, $key);
        Yii::$app->cache->add(self::SESSION_KEY_LIST, $keyList);
    }
    ...
    public function cleanSessionList()
    {
        ...
        Yii::$app->cache->add(self::SESSION_KEY_LIST, []);
    }
}
...
class CourseEnrollService extends LnCourseEnroll
{
     ...
     * 批量设置报名状态[新报名收费]
     * @param $courseId 课程id
     * @param $enrollIds 报名表ids(原来)
	 * @param $enrollNewIds 报名表ids（现在）
     * @param $type 类型
     * @param $approvedId 审批者id
     * @return bool|string
	 * @author xinpeng 20180104
     */
    public function BatchSetBoeEnrollStatus($courseId, $enrollIds,$enrollNewIds, $type, $approvedId)
    {
		...
		$enroll_number = BoeEnroll::find(false)->where(array(
					  'event_type' =>'1',
					  'event_id' =>$courseId,	
		...
		/*获取已报名成功的人数以及报名人数的限制*/
		if(($enroll_number+$enrollCount) > $course->limit_number&&$type == LnCourseEnroll::ENROLL_TYPE_ALLOW)
		{
			return Yii::t('boe', 'enroll_number_limit');
		}
        foreach ($enrollIds as $i_key=>$id) {
            ...
            if ($data) {
                ...
                if ($type == LnCourseEnroll::ENROLL_TYPE_ALLOW) {/*通过*/
                    ...
                    if ($data->save()) {
						$enrollNewData	= array(
							'approval_confirm'	=>'1',
							'approved_mark'		=>Yii::t('boe', 'flow_approved_accept'),
							'is_charge'			=>'1',
							'user_id'			=>$data->user_id,
							'enroll_id'			=>$enrollNewIds[$i_key],
							'uid'				=>$approvedId,
							'code'				=>50
						);
						BoeEnrollService::updateEnroll($enrollNewData);/*处理新报名流程信息*/
                        ...
                        /*发送邮件*/
						$pushService->sendMailByCourseResult($course, $approvedId, $data->user_id);
                    } else {
                        return '更新数据失败';
                    }
                    ...
                    if ($data->save()) {
                        $enrollNewData	= array(
							'approval_confirm'	=>'2',
							'approved_mark'		=>Yii::t('boe', 'flow_approved_deny'),
							'is_charge'			=>'1',
							'user_id'			=>$data->user_id,
							'enroll_id'			=>$enrollNewIds[$i_key],
							'uid'				=>$approvedId,
                        ...
                        /*发送邮件通知学员*/
						$pushService->sendMailByCourseResult($course, $approvedId, $data->user_id, 2,$enrollNewData['approved_mark']);
                    } else {
                        return '更新数据失败';
                    }
                }
    ...
    public function BatchSetEnrollStatus($courseId, $enrollIds, $type, $approvedId)
    {
        ...
        /** 取消管理员审核名额限制 2016-11-21 Alex Liu*/
        /**
...
//                        $courseService->sendWechatMessageToEnrollUser($data->course_id, $data->user_id, 'courseDisallowToUser');
    ...
    public function courseEnroll($courseId, $userId){
		...
		if($data->is_charge_approval == '1')
		{
			//新报名按照报名成功量来计算剩余报名名额
			$count = BoeEnroll::find(false)->where(array(
				'event_type' =>'1',
				'event_id' =>$id,	
				'enroll_status_code'=>50,
				'is_deleted'=>'0'

...
class ExaminationService extends LnExamination
{
    ...
    const LEARNING_DURATION = '15'; /*更新时长*/
...
//        $result = $paramResult->value;
     ...
     */
    public function GetExaminationByUserOne($examination_id, $userId = null, $company_id = null, $withSession = true){
        $model = LnExamination::findOne($examination_id);
        $examination_question_number = LnExaminationPaperUser::find(false)->andFilterWhere(['examination_id' => $examination_id,'examination_paper_copy_id' => $model->examination_paper_copy_id, 'user_id' => $userId])->select('examination_question_number')->scalar();
        $result = $model->attributes;
        $result['examination_question_number'] = $examination_question_number;
        return $result;
    }
...
//           $result = $query->andFilterWhere(['in','examination_status',array('1','2')])
    ...
    public function GetExaminationInfoByModResId($modResId, $mode,$userId=null,$companyId=null){
        ...
        $examinationModel = $this->GetExaminationByUserOne($courseactivityModel->object_id, $userId);
    ...
    public function GetExaminationPaperQuestionUser($examination_id, $examination_paper_user_id,array $question_fields = [],array $option_fields = []){
        ...
        $examQuestionUserkidArray = TArrayHelper::map($userPaperQuestionModel, 'examination_question_user_id', 'examination_question_user_id');
        $examQuestionUserkidArray = array_keys($examQuestionUserkidArray);
        $examQuestionUserData = $this->getExamQuestionUser($examQuestionUserkidArray, $field, 'qu_id', true);
        $optionsAll = $this->GetResultUserDetail($examQuestionUserkidArray, null, [], false);
        foreach ($userPaperQuestionModel as $key => $item){
            if ($item['relation_type'] == LnExamPaperQuestion::RELATION_TYPE_HR){
                $item['options'] = null;
                $page ++;
            }else{
                $questionCopyInfo = $examQuestionUserData[$item['examination_question_user_id']];
...
//                    $options = $this->GetQuestionOptionsUser($item['examination_question_user_id'],null,$option_fields);
                    $item['options'] = $optionsAll[$item['examination_question_user_id']];
                }
            }
            $result[] = $item;
        }
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public function getExamQuestionUser($kid, $field = [], $index = null, $isArray = false){
        $model = LnExamQuestionUser::find(false);
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public function GetQuestionOptionsUserByKid($kids, array $option_fields = null){
        $query = LnExamQuestOptionUser::find(false)
            ->andFilterWhere(["kid" => $kids])
            ->orderBy('sequence_number');
        if(!empty($option_fields)) {
            $query = $query->select($option_fields);
        }
        return $query->indexBy('kid')->all();
    }
...
//            'res_complete_id' => $params['res_complete_id'],
        ...
        if (!empty($options)){
            ...
            //$insertResultDetailSql = [];
            $updateExamQuestionUserModel = [];
            $examination_question_user_id = array_keys($options);
            $examQuestionUserAll = $this->getExamQuestionUser($examination_question_user_id, [], 'kid');
            $examQuestOptionUserId = TArrayHelper::getArrayVal($options);
            $examQuestOptionUserAll = $this->GetQuestionOptionsUserByKid($examQuestOptionUserId);
            //$examQuestOptionUserAll = TArrayHelper::index($examQuestOptionUserAll, 'kid');
            foreach ($options as $key => $item){
                $question_score = 0;
                $questionUser = $examQuestionUserAll[$key];
                /*多选题*/
                if ($questionUser->examination_question_type == LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_CHECKBOX && is_array($item)){
                    ...
                    $is_right = 1;
                    ///$checkBoxOptions = LnExamQuestOptionUser::find(false)->andFilterWhere(['in', 'kid', $item])->indexBy('kid')->all();
                    foreach ($item as $val) {
                        $isRightOptions = $examQuestOptionUserAll[$val];
//                        $isRightOptions = LnExamQuestOptionUser::findOne($val);
                        ...
                        //array_push($insertResultDetailSql, $result);
                    ...
                    $isRightOptions = $examQuestOptionUserAll[$item];
                    ...
                    if($examinationModel->examination_range == 1) {//课程下考试
                        $questionOptionUserOne = LnExamQuestOptionUser::find(false)->andFilterWhere(['examination_question_user_id' => $key])->limit(1)->one();
                        ...
                        $question_detail = LnExamResultDetail::find(false)->andFilterWhere(['examination_question_user_id'=>$key])->one();
                        if($question_detail->option_result == 'yes'){
                            $correct_number++;
                            $question_score = $questionUser->default_score;
                        }else{
                            $question_score = 0;
...
//                LnExamQuestionUser::updateAll(['examination_question_score' => $question_score, 'submit_status' => '1'],'kid=:kid',[':kid'=>$key]);
                $questionUser->examination_question_score = $question_score;
                $questionUser->submit_status = LnExamQuestionUser::SUBMIT_STATUS_YES;
                $updateExamQuestionUserModel[] = $questionUser;
                $score += $question_score;
            }
            $errMsg = "";
            //BaseActiveRecord::batchInsertSqlArray($insertResultDetailSql, $errMsg);
            BaseActiveRecord::batchUpdateNormalMode($updateExamQuestionUserModel, $errMsg);
            /*转换成百分制*/
     ...
     */
    public function saveResultUserDetail($examination_question_user_id, $params, $examination_option_user_id, $findResultProcessUser, $findResultFinalUser, $company_id, $user_id, $questionUser, $isRightOptions, $examination_version, $option_result = "", $temp = true){
        ...
        if ($temp){
            ...
            return $lnExaminationResultUserDetail;
        }
    }
    ...
    public function GetExaminationUserPaper($result_user_id,array $question_fields = [],array $option_fields = []){
        ...
        $result = [];
        $page = 0;
        $examination_question_user_id = TArrayHelper::map($paperUserQuestion, 'examination_question_user_id', 'examination_question_user_id');
        $examination_question_user_id = array_keys($examination_question_user_id);
        $optionsAll = $this->GetResultUserDetail($examination_question_user_id,$result_user_id,$option_fields);
        foreach ($paperUserQuestion as $key => $item){
            if ($item['relation_type'] == LnExamPaperQuestion::RELATION_TYPE_HR){
                ...
                if ($item['examination_question_type'] == LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_RADIO){
                    $find = LnExamResultDetail::find(false)->andFilterWhere(['examination_question_user_id' => $item['examination_question_user_id'], 'examination_result_process_id' => $result_user_id, 'examination_question_type' => LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_RADIO])->one();
                    ...
                    $count = LnExamQuestOptionUser::find(false)->andFilterWhere(['examination_question_user_id' => $item['examination_question_user_id'], 'is_right_option' => LnExamQuestionOption::IS_RIGHT_OPTION_YES])->count();
                    ...
                    $findCount_corrent = LnExamResultDetail::find(false)->andWhere("examination_option_user_id in ({$sql})")->andFilterWhere(['examination_result_process_id' => $result_user_id, 'option_result' => LnExamResultDetail::OPTION_RESULT_NO, 'examination_question_type' => LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_CHECKBOX])->count();
                    ...
                    //var_dump($item);
                    $find = LnExamResultDetail::find(false)->andFilterWhere(['examination_question_user_id' => $item['examination_question_user_id'], 'examination_result_process_id' => $result_user_id, 'examination_question_type' => LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_JUDGE])->limit(1)->one();
                    $findQuestionOptionUser = $optionsAll[$item['examination_question_user_id']];
                    if (!empty($find) && $find->option_result == self::IS_RIGHT_YES){
                        $item['is_yes'] = 1;
                    }else{
                        $item['is_yes'] = 0;
                    }
                    if ($find->option_stand_result == LnExamQuestionOption::IS_RIGHT_OPTION_YES) {
                        if ($item['is_yes']) {
                            $item['values'] = 1;
                        } else {
                            $item['values'] = 0;
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public function GetResultUserDetail($examination_question_user_id, $examination_result_process_id,array $option_fields = [], $checkCount = true){
        $query = LnExamQuestOptionUser::find(false)
            ->andFilterWhere(["examination_question_user_id" => $examination_question_user_id])
            ->orderBy('sequence_number');
        if(!empty($option_fields)) {
        ...
        $result = [];
        if ($lnExamQuestionOption && $checkCount){
        ...
        if (!empty($result)){
            $res = TArrayHelper::index($result, null, 'examination_question_user_id');
        }else{
            $res = [];
        }
        return $res;
    ...
    public function GetResultUserDetailSelect($userId, $companyId, $examinationId, $processId, $field = 'examination_option_user_id'){
        $result = LnExamResultDetail::find(false)->andFilterWhere(['examination_result_process_id' => $processId])->select($field)->asArray()->all();
        if ($result){
 ...
 /**
    ...
    /**
     * @param $paper_id
     * @param string $examination_mod
     * @param string $option_disorder
     * @return array|\yii\db\ActiveRecord[]
     */
...
//                    }
    ...
    public function getRelatedUserCount($examinationId){
        $count = LnRelatedUser::find(false)->andFilterWhere(['learning_object_id' => $examinationId, 'learning_object_type' => LnRelatedUser::OBJECT_TYPE_EXAM, 'status' => LnRelatedUser::STATUS_FLAG_NORMAL])->distinct('user_id')->count();
        return $count;
    }
    ...
    public function getExaminationUserInfoResult($id, $params, $callback = 'query'){
        ...
        if (!empty($params['user_keyword'])){
            ...
            $where_1 = " and (real_name like '%{$user_keyword}%' OR email like '%{$user_keyword}%' OR user_no like '%{$user_keyword}%')";
        }
...
LEFT JOIN
(
SELECT kid as result_id,user_id as uid,examination_score,examination_status,correct_rate,result_type from (select * from {$resultUserTable} WHERE examination_id='{$id}' and result_type='{$params['result_type']}' and is_deleted='0' ORDER BY examination_status desc ) t5 GROUP BY user_id
) t2
...
on t5.user_id=t4.kid WHERE t4.kid is not null GROUP BY t4.kid";
    ...
    public function getExaminationPlayResult($processResultId, $userId, $companyId, $modResId = null, $mode = ''){
        ...
        /*考试基本信息*/
        $examination = $this->GetExaminationByUserOne($userResult->examination_id, $userId);
        /*获取当前的结果集*/
        ...
        if ($mode == 'course'){
            ...
            $resCompleteProcessModel = LnResComplete::find(false)->andFilterWhere(['course_id'=>$userResult->course_id, 'user_id'=> $userId, 'mod_id'=> $userResult->mod_id, 'mod_res_id' => $modResId, 'courseactivity_id' => $userResult->courseactivity_id, 'complete_type' => LnResComplete::COMPLETE_TYPE_PROCESS])->limit(1)->one();
    ...
    public function player($userId, $companyId, $courseId, $coursewareId, $modResId, $courseRegId, $courseCompleteFinalId, $attempt, $mode = self::PLAY_MODE_NORMAL){
        ...
        $examination = $this->GetExaminationInfoByModResId($modResId, $mode, $userId);
        $examinationModel = $examination['examination'];

        $generate = true;
        $errMessage = "";
        //判断生成学员试卷数据
        if (empty($examinationLast) && $mode == self::PLAY_MODE_NORMAL) {
...
//                    echo $errMessage;
            ...
            /*更新res_complete数据记录*/
            if (!empty($examinationLast) && $examinationModel['examination_mode'] == LnExamination::EXAMINATION_MODE_TEST) {
    ...
    public function tempSave($params, $company_id, $user_id){
        ...
        if ($examQuestionUser->examination_question_type == LnExaminationQuestion::EXAMINATION_QUESTION_TYPE_RADIO){
            ...
            if ($findResultUserDetail){
		        ...
		        /*保存临时数据*/
                $this->saveResultUserDetail($examination_question_user_id, $params, $options_id, $resultProcessUser, $resultFinalUser, $company_id, $user_id, $examQuestionUser, $examinationQuestionOptionUser, $examinationModel->examination_version, $option_result, true);
            }
            ...
            if ($findResultUserDetail){
		        ...
		        /*删除以前的临时记录*/
                LnExamResultDetail::physicalDeleteAll(['user_id' => $user_id, 'examination_id' => $examination_id, 'examination_question_user_id' => $examination_question_user_id, 'examination_result_process_id' => $resultProcessUser->kid, 'examination_result_final_id' => $resultFinalUser->kid]);
                $this->saveResultUserDetail($examination_question_user_id, $params, $examinationQuestionOptionUser->kid, $resultProcessUser, $resultFinalUser, $company_id, $user_id, $examQuestionUser, $examinationQuestionOptionUser, $examinationModel->examination_version, $option_result, true);
            }

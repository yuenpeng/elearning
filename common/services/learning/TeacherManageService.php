...
// 	}
	...
	public function editTeacherStatus($id,$status)
	{
		$attributes = [	
				'status' => $status
		];
		
		$teacher = new LnTeacher();
		$teacher->updateAll($attributes, "kid = '" . $id . "'");
	...
	public function courseSearchTeacher($keyword = null, $kids = null, $input = true, $companyId = null)
	{
		...
		$query = LnTeacher::find(false);
        $query->andFilterWhere(['=', LnTeacher::tableName() . '.status', LnTeacher::STATUS_FLAG_NORMAL]);

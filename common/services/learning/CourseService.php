...
//        }
    ...
    public function TagCourseSearch($keyword)
    {
        $tagdata = FwTagReference::find(false)
            ->andFilterWhere(['=', FwTagReference::tableName() . '.tag_value', $keyword])
            ->andFilterWhere(['=', FwTagReference::tableName() . '.status', FwTagReference::STATUS_FLAG_NORMAL])
            ->all();

        $tagcourseid = TArrayHelper::map($tagdata, 'subject_id', 'subject_id');
        $tagcourseids = array_keys($tagcourseid);
        ...
        if (!empty($domain)) {
            ...
            $query->innerJoinWith('lnResourceDomains')
                ->andFilterWhere(['in', LnResourceDomain::tableName() . '.domain_id', $domain])
                ->andFilterWhere(['in', LnCourse::tableName() . '.kid', $tagcourseids])
                ->andFilterWhere(['=', LnResourceDomain::tableName() . '.resource_type', LnResourceDomain::RESOURCE_TYPE_COURSE])
                ->andFilterWhere(['=', LnResourceDomain::tableName() . '.status', LnResourceDomain::STATUS_FLAG_NORMAL])
                ->distinct();
        } else {
    ...
    public function TagQuestionSearch($keyword)
    {
        $tagdata = FwTagReference::find(false)
            ->andFilterWhere(['=', FwTagReference::tableName() . '.tag_value', $keyword])
            ->andFilterWhere(['=', FwTagReference::tableName() . '.status', FwTagReference::STATUS_FLAG_NORMAL])
        ...
        $dataProvider = new ActiveDataProvider([
            ...
            $query->andFilterWhere(['in', SoQuestion::tableName() . '.kid', $tagcourseids])
                ->andFilterWhere(['=', SoQuestion::tableName() . '.company_id', $company_id])
                ->distinct();


        $dataProvider->setSort(false);
        $query->addOrderBy([SoQuestion::tableName() . '.created_at' => SORT_DESC]);
      // echo ($query->createCommand()->getRawSql());
        return $dataProvider;
    }
...
//        $courseModel = LnCourse::findOne(['kid'=>$courseId],false);
     ...
     * @return array|null|yii\db\ActiveRecord
     */
    public function getUserRegInfoRegState($uid, $courseId)
    {
//        $courseModel = LnCourse::findOne(['kid'=>$courseId],false);
        $model = new LnCourseReg();
        $query = $model->find(false)
     ...
     * @return bool
     */
    public function isUserRegCourseRegState($uid, $courseId, &$regCourseId, $withSession = true)
    {
        if (!empty($uid)) {
            $sessionKey = "UserCourseRegState_UserId_" . $uid . "_CourseId_" . $courseId;

            if ($withSession && Yii::$app->session->has($sessionKey)) {
                $regCourseId = Yii::$app->session->get($sessionKey);
                return true;
            } else {
                $result = $this->getUserRegInfoRegState($uid, $courseId);
...
//            ->andWhere(['t1.complete_status' => LnCourseComplete::COMPLETE_STATUS_DONE])
    ...
    public function getLastLearnCourse($uid)
    {
        ...
        $query = LnCourse::find(false);
        $query->andFilterWhere(['=', 'kid', $complete_data->course_id]);
        $data = $query->select('kid,course_name,theme_url')
            ->asArray()
            ->one();
     ...
     * @return bool
     */
    public function regCourse($uid, $courseId, $ret_type, $sponsor_id = null, $oper = false, $lcreg = null)
    {
        // 手动报名 跳过受众验证
        if (!$oper) {
            /*判断是否在受众范围内*/
        ...
        if ($courseData->course_type == LnCourse::COURSE_TYPE_ONLINE) {
            ...
            /*if ($oper) {
                ...
                $reg_state = LnCourseReg::REG_STATE_APPLING;
            /*}*/
			//判断是收费审批课程的情况下，需要增加新报名流程信息
			if($courseData->is_charge_approval == '1')
			{
				$en_data	= array(
					'course_id'	=>$courseId,
					'user_id'	=>$uid,
				);
				$en_res = BoeEnrollService::addEnroll($en_data);
			}
        }
...
//    }
    ...
    public function GetCourseByTrainAddress($addressid,$companyId,$starttime,$endtime,$size, $page)
    {

        $model = new LnCourse();
        $result =  $model->find(false);
        if ($companyId == null) {
            $result->andWhere(LnCourse::tableName() . '.company_id is null');
        } else {
            $result->andWhere(['=', LnCourse::tableName() . '.company_id', $companyId]);
        }
        if ($starttime != null) {
            $result->andWhere(['>', LnCourse::tableName() . '.open_start_time', ($starttime - 86400)]);
        }
        if ($endtime != null) {
            $result->andWhere(['<', LnCourse::tableName() . '.open_end_time', ($endtime + 86400)]);
        }
        $result->andWhere(['=',LnCourse::tableName() . '.training_address_id', $addressid])
               ->addOrderBy([LnCourse::tableName() . '.open_start_time' => SORT_DESC])
                ->limit($size)
                ->offset($this->getOffset($page, $size));

        return $result->all();
    }
    public function CountCourseByTrainAddress($addressid,$companyId,$starttime,$endtime,$size, $page)
    {
        ...
        $result->andWhere(['=',LnCourse::tableName() . '.training_address_id', $addressid])
            ->addOrderBy([LnCourse::tableName() . '.created_at' => SORT_DESC])
            ->limit($size)
            ->offset($this->getOffset($page, $size));
        ...
        return $result->count('kid');
    }
    public function GetCourseByVendor($addressid,$companyId,$starttime,$endtime,$size, $page)
    {
        ...
        $result->andWhere(['=',LnCourse::tableName() . '.vendor_id', $addressid])
               ->addOrderBy([LnCourse::tableName() . '.open_start_time' => SORT_DESC])
                ->limit($size)
                ->offset($this->getOffset($page, $size));
    ...
    public function CountCourseByVendor($addressid,$companyId,$starttime,$endtime,$size, $page)
    {
...
//            ->addOrderBy([LnModRes::tableName() . '.updated_at' => SORT_ASC])
     ...
     */
    public function teacherGetCourse($uid, $type = 'before',$search = '')
    {
        $currentTime = time();
        ...
        if(!empty($search)) {
            $courseModel->andWhere(['like', LnCourse::tableName() . '.course_name', $search]);
        }
        if ($type == "start") {
    ...
    public function searchCourseReg($courseId, $params = null)
    {
        ...
        $enrollModel->from(LnCourseReg::tableName() . ' as len')
            ...
            ->distinct()
            ->select('t1.real_name,t1.user_no,t1.orgnization_name,t1.position_name,t1.email,t1.mobile_no,t1.location,len.kid,len.user_id,len.reg_time');
...
//            ->andFilterWhere(['<>','enroll_type',LnCourseEnroll::ENROLL_TYPE_DISALLOW])
        ...
        if ($enrollResult > 0) {
            ...
            } elseif ($model->enroll_type === LnCourseEnroll::ENROLL_TYPE_REG) {
            ...
            } elseif ($model->enroll_type === LnCourseEnroll::ENROLL_TYPE_ALTERNATE) {
                $approved_state = LnCourseEnroll::APPROVED_STATE_APPLING;
            } else {
                $approved_state = LnCourseEnroll::APPROVED_STATE_APPROVED;
            }
    ...
    {
        $enrollModel = new LnCourseEnroll();
        $model = $enrollModel->find(false)
            ->andFilterWhere(['=', 'course_id', $data['course_id']])
            ->andFilterWhere(['=', 'user_id', $data['user_id']])
//            ->andFilterWhere(['<>','enroll_type',LnCourseEnroll::ENROLL_TYPE_DISALLOW])
            ->one();

        if ($model && $model->enroll_type === LnCourseEnroll::ENROLL_TYPE_ALLOW) {
            return ['result' => 'fail', 'errcode' => 'not_allow', 'errmsg' => Yii::t('frontend', 'exam_re_enroll')];/*已经报过名*/
        } else {
            if ($model) {
                $regModel = LnCourseReg::findOne(['course_id' => $model->course_id, 'user_id' => $model->user_id]);
            } else {
                $model = new LnCourseEnroll();
                $regModel = null;
            }
            ...
            $model->cancel_state = null;
            $model->cancel_by = null;
            $model->cancel_at = null;
            $model->cancel_reason = null;
            $model->enroll_time = time();

            /*20160122修改*/
            $ret = $this->regCourse($data['user_id'], $data['course_id'], LnCourseReg::REG_TYPE_MANAGER, null, true, $regModel);/*添加数据到注册表*/
            if ($ret) {
                $model->save();
                return ['result' => 'success'];
            } else {
    ...
    public function GetRegCourseByUserId($user_id, $key, $type, $limit, $offset, $current_time)
    {
        if ($type === 'all') {
            ...
            $query->joinWith('lnCourse')
                ...
                ->andFilterWhere(['=', LnCourse::tableName() . '.is_deleted', LnCourse::DISPLAY_FLAG_NO])
...
//                                $catalogMenu .= $beginItemUnLearnStr;
    ...
    public function detail($user_id, $course_id, $is_manager = false, $require_menu = false, $require_enroll_info = false, $mode = self::PLAY_MODE_NORMAL, $require_score = false, $sort_menu = false)
    {
        ...
        $courseRegId = null;
        /*判断是否注册*/
        $isReg = $this->isUserRegCourseRegState($user_id, $course_id, $courseRegId);
...
//        $courseCompleteService->initCourseCompleteInfo($courseRegId, $course_id, $user_id);
    ...
    public function getCourseIdByCategoryId($category_id){
        return LnCourse::find()->select('kid')->where(['category_id'=>$category_id])->asArray()->all();
    }
    ...
    public function getCategoryIdByName($category_name){
       $category =  (new yii\db\Query())->select('kid')->from('eln_ln_course_category')->where(['category_name'=>$category_name])->one();
        return $category['kid'];
    }
     ...
     * 查询已报名的课程
     * @param $category_id
     * @param string $reg_type
     * @return array|yii\db\ActiveRecord[]
     */

    public function getMyAutoCourse($userId,$category_id){
        $query = LnCourse::find(false);
        $query->innerJoinWith('lnCourseReg')
            ->andWhere(["=",LnCourse::realTableName().'.category_id',$category_id])
            ->andWhere(["=",LnCourseReg::realTableName().'.user_id',$userId])
            ->andWhere(['course_type'=>LnCourse::COURSE_TYPE_ONLINE])
            ->select([LnCourse::realTableName().'.course_name',LnCourseReg::realTableName().'.course_id',LnCourseReg::realTableName().'.kid']);
     ...
     * 获取课程数量
     */
    public function getCategoryRegCourseCount($user_id,$category_ids,$type,$current_time){
        if ($type === 'all') {
            $query = LnCourse::find(false);
            $query->andFilterWhere(['<', LnCourse::tableName() . '.created_at', $current_time])
                ->andFilterWhere(['in',LnCourse::tableName(). '.category_id', $category_ids])
                ->andFilterWhere(['or',['>',LnCourse::tableName(). '.end_time',$current_time],['<',LnCourse::tableName(). '.end_time',1]])
                ->andFilterWhere(['=',LnCourse::tableName(). '.status',1]);
        } elseif ($type === LnCourseComplete::COMPLETE_STATUS_DONE) {
            $query = LnCourseComplete::find(false);
            $query->joinWith('lnCourse')
                ->leftJoin(LnCourseReg::tableName() . ' reg', LnCourseComplete::tableName() . '.course_reg_id=reg.kid')
                ->andFilterWhere(['=', LnCourseComplete::tableName() . '.user_id', $user_id])
                ...
                ->andFilterWhere(['or',['>',LnCourse::tableName(). '.end_time',$current_time],['<',LnCourse::tableName(). '.end_time',1]])
                ->andFilterWhere(['=',LnCourse::tableName(). '.status',1]);
        } elseif ($type === LnCourseComplete::COMPLETE_STATUS_NOTSTART) {
            $query = LnCourseComplete::find(false);

            $completeSql = $query->andFilterWhere(['=', 'user_id', $user_id])
                ->andFilterWhere(['=', 'complete_type', LnCourseComplete::COMPLETE_TYPE_FINAL])
                ->andFilterWhere(['or', ['or',['=', 'complete_status', LnCourseComplete::COMPLETE_STATUS_DONE], ['=', 'is_retake', LnCourseComplete::IS_RETAKE_YES]],
                    ['and',['=','is_retake', LnCourseComplete::IS_RETAKE_NO],['=','complete_status', LnCourseComplete::COMPLETE_STATUS_DOING]]])
                ->select('course_id')
                ->distinct()
                ->createCommand()->rawSql;

            $query = LnCourse::find(false);
            $query->where('kid not in (' . $completeSql . ')')
                ->andWhere(['=','is_deleted',0])
                ->andFilterWhere(['<', 'created_at', $current_time])
                ->andFilterWhere(['in','category_id' ,$category_ids])
                ->andFilterWhere(['or',['>','end_time',$current_time],['<','end_time',1]])
                ->andFilterWhere(['=','status',1]);

        }elseif($type === LnCourseComplete::COMPLETE_STATUS_DOING){
            $query = LnCourseComplete::find(false);
            $query->joinWith('lnCourse')
                ->leftJoin(LnCourseReg::tableName() . ' reg', LnCourseComplete::tableName() . '.course_reg_id=reg.kid')
                ->andFilterWhere(['=', LnCourseComplete::tableName() . '.user_id', $user_id])
                ->andFilterWhere(['=', LnCourseComplete::tableName() . '.complete_type', LnCourseComplete::COMPLETE_TYPE_FINAL])
                ->andFilterWhere(['and',
                    ['=', LnCourseComplete::tableName() . '.complete_status', LnCourseComplete::COMPLETE_STATUS_DOING],
                    ['=', LnCourseComplete::tableName() . '.is_retake', LnCourseComplete::IS_RETAKE_NO]])
                ->andFilterWhere(['in',LnCourse::tableName() . '.category_id', $category_ids])
                ->andFilterWhere(['or',['>',LnCourse::tableName(). '.end_time',$current_time],['<',LnCourse::tableName(). '.end_time',1]])
                ->andFilterWhere(['=',LnCourse::tableName(). '.status',1]);
        }
    ...
    public function getCategoryRegCourseByUserId($user_id,$category_ids,$type, $limit, $offset, $current_time,$is_count = false)
    {
        if ($type === 'all') {
            $query = LnCourse::find(false);
            $query->andFilterWhere(['<', LnCourse::tableName() . '.created_at', $current_time])
                ...
                ->andFilterWhere(['=',LnCourse::tableName(). '.status',1])
                ->andFilterWhere(['or',['>',LnCourse::tableName(). '.end_time',$current_time],['<',LnCourse::tableName(). '.end_time',1]])
                ->orderBy(LnCourse::tableName() . '.created_at desc');
        } elseif ($type === LnCourseComplete::COMPLETE_STATUS_DONE) {
            $query = LnCourseComplete::find(false);
        ...
        if($is_count){
            ...
            $data = $query
                ...
                ->asArray()
                ->all();
            if($type === LnCourseComplete::COMPLETE_STATUS_NOTSTART || $type == 'all'){
                foreach($data as $key=>$value){
                    $data[$key]['lnCourse'] = $value;
                }
            }
    ...
    static public function CourseInCategory($course_category,$category_id = false){
        $category_name = "管理进阶课";
        $category = LnCourseCategory::find(false)->where(['category_name'=>$category_name])->one();
        $courseCategoryService = new CourseCategoryService();
        $categories = $courseCategoryService->getSubCategories($category->kid);
    ...
    public function getCourseByCategoryId($category_id,$current_time){
        if(!$category_id){
            return [];
        }
        $query = LnCourse::find(false);
        $query->andFilterWhere(['<', 'created_at', $current_time])
            ->andFilterWhere(['in','category_id', $category_id])
            ->andFilterWhere(['=','status',1])
            ->andFilterWhere(['or',['>','end_time',$current_time],'end_time is null'])
            ->select('kid,course_name,course_period,course_period_unit,register_number,theme_url')
            ->orderBy('created_at desc');
        $courseList = $query->asArray()->all();
        return $courseList;
    }

    public function getCourseByIds($ids,$current_time,$limit = 100,$offset = 100){
        if(empty($ids)){
            return [];
        }
        $query = LnCourse::find(false);
        $query->andFilterWhere(['<', 'created_at', $current_time])
            ...
            ->andFilterWhere(['=','status',1])
            //->andFilterWhere(['or',['>','end_time',$current_time],'end_time is null'])
            ->select('kid,course_name,theme_url,course_desc_nohtml,release_at,course_code')
            ->orderBy('created_at desc');
        if($limit !== 100){
    ...
    public function getCourseCountByIds($ids,$current_time){
        $query = LnCourse::find(false);
        return $query->andFilterWhere(['<', 'created_at', $current_time])
            ->andFilterWhere(['in','kid',$ids])
            ->andFilterWhere(['=','status',1])
            ->count();
    ...
    public function getRegedCourseByCategoryId($user_id,$category_name='Grow180-HRBP培训计划'){
        $course_category = LnCourseCategory::findOne(['category_name'=>$category_name]);
        if($course_category && !empty($course_category)){
            $query = LnCourseReg::find(false);
            $query->leftJoin(LnCourse::tableName() . 'course', LnCourseReg::tableName() . '.course_id=course.kid')
                ->andFilterWhere(['=', LnCourseReg::tableName() . '.user_id', $user_id])
                ->andFilterWhere(['=','course.status',1])
                ->andFilterWhere(['=','course.category_id',$course_category->kid])
                ->andFilterWhere(['=','course.is_deleted',0])
                ->select('course.kid,course.course_name');
            $course_list = $query->asArray()->all();
            return $course_list;
        }
        return false;

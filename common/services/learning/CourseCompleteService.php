...
//        }
    ...
    public function checkCourseCompleteInfoExist($courseRegId,$completeType, $withSession = true)
    {
        ...
        if ($withSession && Yii::$app->session->has($sessionKey)) {
			...
			{
				$log_data = "课程完成数据重复标记_".$sessionKey;
				Yii::info($log_data,"ywlog");
				unset($log_data);
			}else{
				$log_data = "课程完成数据[session]_".$sessionKey;
				Yii::info($log_data,"ywlog");
				unset($log_data);
			}
			//记录日志E-01
            ...
            if (isset($result) && intval($result) > 0) {
                //记录日志B-02
				$log_data = "课程完成数据[查询数据库]_1_".$sessionKey."_".$result;
				Yii::info($log_data,"ywlog");
				unset($log_data);
				//记录日志E-02
				if ($withSession) {
                ...
                //记录日志B-03
				$log_data = "课程完成数据[查询数据库]_0_".$sessionKey."_".json_encode($result);
				Yii::info($log_data,"ywlog");
				unset($log_data);
				//记录日志E-03
				return false;
    ...
    public function initCourseCompleteInfo($courseRegId, $courseId, $user_id = null)
    {
        $model = new LnCourseComplete();
            ...
            {
			//记录日志B
            $sessionKey = "CourseReg_".$courseRegId."_CompleteType_1";
			$has_key    = Yii::$app->session->has($sessionKey);
			$get_key	= $has_key?Yii::$app->session->get($sessionKey):false;
			$complete_sql	= "select boe_get_complete_count('".$courseRegId."') as num";
			$connection		= Yii::$app->db;
			$complete_data	= $connection->createCommand($complete_sql)->queryOne();
			$log_data = "课程完成数据[插入数据库(完成数：".$complete_data['num'].")]_".$sessionKey."_".$has_key."_".$get_key."_".json_encode($result);
			Yii::info($log_data,"ywlog");
			unset($log_data,$complete_sql,$connection,$complete_data);
			//记录日志E
			if(!$has_key || !$get_key)
			{
				...
				/*
				 * 新插入数据信息20180417-xinpeng-B
				*/
				$isPassed		= $courseModel->course_type == LnCourse::COURSE_TYPE_ONLINE ? LnCourseComplete::IS_PASSED_NO : LnCourseComplete::IS_PASSED_YES;
				$completeStatus = LnCourseComplete::COMPLETE_STATUS_NOTSTART;
				$isDirectCompleted = LnCourseComplete::IS_DIRECT_COMPLETED_NO;
				$createdBy	= Yii::$app->user->getId();
				$createdIP	= Yii::$app->request->userIP;
				
                $transaction = Yii::$app->getDb()->beginTransaction();
                try {
                    $command = Yii::$app->getDb()->createCommand("CALL boe_insert_course_complete(:courseId,:courseRegId,:completeStatus,:userId,:isPassed,:isDirectCompleted,:courseVersion,:createdBy,:createdIP)")
                        ->bindValues(array(':courseId' => $courseId, ':courseRegId' => $courseRegId ,':completeStatus' => $completeStatus,':userId' => $userId,':isPassed' => $isPassed,':isDirectCompleted' => $isDirectCompleted,':courseVersion' => $courseVersion,':createdBy' => $createdBy,':createdIP' => $createdIP));
                    $command->execute();
                    $transaction->commit();
                } catch (Exception $e) {
				...
				*/
				...
				$connection		= Yii::$app->db;
				$complete_data	= $connection->createCommand($complete_sql)->queryOne();
				if(isset($complete_data['num'])&&$complete_data['num']>=1)
				{
					Yii::$app->session->set($sessionKey, true);
				}
				$log_data = "课程完成数据[插入结果(完成数：".$complete_data['num'].")],返回：".json_encode($res);
				Yii::info($log_data,"ywlog");
				unset($log_data,$complete_sql,$connection,$complete_data,$res);
				//记录日志E
			}

...
{
    const RECORD_TEMPLATE_COURSE_1 = '我{verb}了{category}《{acivity}》';
    const RECORD_TEMPLATE_COURSE_2 = '我{verb}了{category}《{acivity}》，获得{result}个学分';
    const RECORD_TEMPLATE_COURSE_3 = '{who}给你{verb}了{category}《{acivity}》';
    const RECORD_TEMPLATE_COURSE_4 = '我{verb}参加课程《{acivity}》审核通过';
    const RECORD_TEMPLATE_QUESTION_1 = '我{verb}了关于"{acivity}"的问题';
    const RECORD_TEMPLATE_QUESTION_2 = '我{verb}【{result}】回答关于“{acivity}”的问题';
    const RECORD_TEMPLATE_RECORD_1 = '我{verb}了{category}"{acivity}"';
    const RECORD_TEMPLATE_EXAM_1 = '{who}给你{verb}了{category}《{acivity}》';
    const RECORD_TEMPLATE_EXAM_2 = '我{verb}了{category}《{acivity}》，获得{result}分';
    const RECORD_TEMPLATE_SURVEY_1 = '{who}给你{verb}了{category}《{acivity}》';
    const RECORD_TEMPLATE_SURVEY_2 = '我{verb}了{category}《{acivity}》';
    const RECORD_TEMPLATE_CERT_1 = '我{verb}了{category}“{acivity}”';
    ...
    public function getRecordByUserId($user_id, $type, $size, $page, $current_time)
    {
        ...
        // 防止有新数据时，重复读取数据
        $query->select('kid')->andFilterWhere(['<', 'created_at', $current_time]);
        $query->andFilterWhere(['=', 'user_id', $user_id])
            ->andFilterWhere(['=', 'record_category', $type])
            ->orderBy('created_at desc');
        $kids =  $query->asArray()->all();
        $new_query = LnRecord::find(false);
        $new_query->andFilterWhere(['in','kid',$kids])
            ->limit($size)
            ->offset(self::getOffset($page, $size))
            ->groupBy(['learning_verb','learning_acivity']);

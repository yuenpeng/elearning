...
//            echo $query->createCommand()->getRawSql();
    ...
    public function GetCourseware($params)
    {
        ...
        $domain = [];
        if ($domainList){
            $domain = TArrayHelper::map($domainList, 'kid', 'kid');
    ...
    public function GetCoursewareByVendor($addressid,$companyId,$starttime,$endtime,$size, $page)
    {

        $model = new LnCourseware();
        $result =  $model->find(false);
        if ($companyId == null) {
            $result->andWhere(LnCourseware::tableName() . '.company_id is null');
        } else {
            $result->andWhere(['=', LnCourseware::tableName() . '.company_id', $companyId]);
        }
        if ($starttime != null) {
            $result->andWhere(['>', LnCourseware::tableName() . '.start_at', ($starttime - 86400)]);
        }
        if ($endtime != null) {
            $result->andWhere(['<', LnCourseware::tableName() . '.end_at', ($endtime + 86400)]);
        }
        $result->andWhere(['=',LnCourseware::tableName() . '.vendor_id', $addressid])
            ->addOrderBy([LnCourseware::tableName() . '.created_at' => SORT_DESC])
            ->limit($size)
            ->offset($this->getOffset($page, $size));

        return $result->all();
    }
    public function CountCoursewareByVendor($addressid,$companyId,$starttime,$endtime,$size, $page)
    {

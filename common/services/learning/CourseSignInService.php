...
class CourseSignInService extends LnCourseSignIn {
    ...
    function getStudentSignInStatus($courseId, $studentId, $time = 0) {
        ...
        if (!$temp) {
            return ['result' => '2', 'msg' => 'can not find sign_in_setting_id for this time'];
        }
    ...
    public function getEnrollStudentList($courseId, $sp, $pageSize = 12) {
        ...
        if ($sp['keyword'] = trim($sp['keyword'])) {
            $query->andWhere("t1.real_name like '%{$sp['keyword']}%' or t1.orgnization_name like '%{$sp['keyword']}%' or t1.position_name like '%{$sp['keyword']}%' or t1.user_name like '%{$sp['keyword']}%'");
        }
        if ($sp['not_uid']) {
...
//echo $query->createCommand()->getSql();exit;
    ...
    function deleteSignInRecords($courseId, $studentId, $signInSettingId) {
        ...
        $result = LnCourseSignIn::deleteAll("course_id=:course_id AND user_id=:user_id AND sign_in_setting_id=:ssid", [':course_id' => $courseId, ':user_id' => $studentId, ':ssid' => $signInSettingId]);
    ...
    function studentSignIn($signInModel) {
        if (!$signInModel->course_id) {
            return ['result' => '3', 'msg' => Yii::t('frontend', '{value}_not_null', ['value' => Yii::t('common', 'course_id')])];
        }
        if (!$signInModel->user_id) {
            return ['result' => '4', 'msg' => Yii::t('frontend', '{value}_not_null', ['value' => Yii::t('common', 'user_id')])];
        }

        $courseModel = LnCourse::findOne($signInModel->course_id);
        if (!$courseModel) {
            return ['result' => '5', 'msg' => Yii::t('frontend', '{value}_not_exists', ['value' => Yii::t('common', 'course')])];
        }

        // 检查学员是否已报名或者报名未审核通过
        $enroll = new LnCourseEnroll();
        $enroll_data = $enroll->findOne(array('course_id' => $signInModel->course_id, 'user_id' => $signInModel->user_id));
        if ($enroll_data) {
            if ($enroll_data->enroll_type != 1) {
                return ['result' => '12', 'msg' => Yii::t('frontend', 'sign_in_not_approved_state')];
            }
        } else {
            if (!$temp) {
                return ['result' => '11', 'msg' => Yii::t('frontend', 'sign_in_not_in_enroll')];
            } // 未报名
        }
        //签到需做开课状态限制
        ...
        if ($temp) {
            if ($signInModel->sign_in_setting_id) {
                if ($signInModel->sign_in_setting_id != $temp->kid) {
                    $signInModel->sign_in_setting_id = null;
                }
            } else {
        ...
        if ($signInModel->sign_in_setting_id) {
            ...
            if ($signRec) {
                if ($signRec['sign_flag'] == LnCourseSignIn::SIGN_FLAG_SIGN_IN) {
                    if ($signInModel->sign_flag == LnCourseSignIn::SIGN_FLAG_LEAVE) {
                        return ['result' => '6', 'msg' => Yii::t('frontend', 'signin_is_not_repeat_left')]; //已签到 无需请假
                    } else {
                        return ['result' => '6', 'msg' => Yii::t('frontend', 'signin_is_not_repeat')];  //已签到无需重复签到
                    }
                } else if ($signRec['sign_flag'] == LnCourseSignIn::SIGN_FLAG_LEAVE) {
                    $msg_name = Yii::t('frontend', 'ask_for_leave');
                    if ($signInModel->sign_flag == LnCourseSignIn::SIGN_FLAG_LEAVE) {
                        return ['result' => '10', 'msg' => Yii::t('frontend', 'sign_in_no_need_because_has_left_left')];    //已请假，无需重复请假
                    } else {
                        return ['result' => '10', 'msg' => Yii::t('frontend', 'sign_in_no_need_because_has_left')]; // 已请假，不能签到
                    }
                } else {
//                    return ['result' => '8', 'msg' => Yii::t('frontend', 'sign_flag_error')];
            ...
            return ['result' => '2', 'msg' => Yii::t('frontend', 'sign_in_time_not_in_range_please_wait')]; //未到签到时间
        }

        //设置默认值
        if (!$signInModel->sign_flag) {
            $signInModel->sign_flag = LnCourseSignIn::SIGN_FLAG_SIGN_IN;
        } //默认签到
        if (!$signInModel->sign_time) {
            $signInModel->sign_time = $nowTime;
        } //默认当前时间
        if (!$signInModel->sign_type) {
            $signInModel->sign_type = LnCourseSignIn::SIGN_TYPE_SELF;
        } //默认自己
        if (!$signInModel->sign_user_id) {
            $signInModel->sign_user_id = Yii::$app->user->identity->getId();
        } //默认自己的id
        if (!$signInModel->sign_system) {

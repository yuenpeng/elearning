...
//                }
//            }
            //20170718 adophper,当scorm为记分课件时设置在100
            if ($allScoPassed && $scoreBefore == null && $isRecordScore) {
                //并且是没有设置过合格线的课件。如果有合格线设置，说明这门课是有一定要求的
                if (!isset($passGrade) || $passGrade == null || $passGrade == 0) {
                    $scoreBefore = $singleTotalScore;
                }
...
//            ->andFilterWhere(['=','complete_status',LnResComplete::COMPLETE_STATUS_DONE])
     ...
     * 根据条件获取相应的数据
     */
    public function getResComplete($where,$desc=null){
        $query = LnResComplete::find(false)->where($where);
        if(!is_null($desc)){
            $query->orderBy($desc);
        }
        return $query->one();
     ...
     * 根据KID更新数据
     */
    public function updateResCompleteById($kid,$attributes){
        $model = new LnResComplete();
        $params = [
            ':kid'=>$kid,
        ];
        $condition = 'kid = :kid';
        $attributes['created_from'] = "repaired";

...
class InvestigationResultService extends LnInvestigation
{
	...
	public function getSingleVoteStUserInfoResult($id, $params) {
		...
		$query_tmp = LnInvestigationResult::find ( false )
			...
			->select ( "user_id,GROUP_CONCAT(option_title  separator '|') as option_title,
					.$m_table.created_at " );
		...
		$query_no_submit_user = "
               ...
               (
           		    SELECT user_id, GROUP_CONCAT(option_title  separator '|') as option_title, created_at FROM eln_ln_investigation_result  r
		...
		if ($rel_users_type == "all") {
			...
			if(trim($params['keyword'])!=""){
				...
				$sql_user_info="(select user_id,real_name,orgnization_name,position_name,user_name from  ".FwUserDisplayInfo::tableName()." where  real_name like '%$keyword%' or user_no like '%$keyword%' or email like '%$keyword%' )";
		...
		if ($rel_users_type == "yes") {
			...
			if(trim($params['keyword'])!=""){
				$keyword=trim($params['keyword']);
				$sql_user_info="(select user_id,real_name,orgnization_name,position_name,user_name from  ".FwUserDisplayInfo::tableName()." where  real_name like '%$keyword%' or user_no like '%$keyword%' or email like '%$keyword%' )";
				$sql_=" SELECT r.user_id, GROUP_CONCAT(option_title  separator '|') as option_title, r.created_at FROM eln_ln_investigation_result  r

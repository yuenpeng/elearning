...
//            ->asArray()
    ...
    public function BuildCategoryTree($data, $parent_category_id = null, $level = 0)
    {
        ...
        foreach ($data as $item) {
            if (empty(trim($item->parent_category_id))) $item->parent_category_id = null;
            ...
            if ($parent_category_id === $item->parent_category_id) {
                ...
                if ($item->is_hot === BaseActiveRecord::YES) {
                    ...
                    $content .= '<li class="hidden"><input type="checkbox" value="' . $item->kid . '" class="category_id"  data-parent="' . $item->node_code_path . '" data-path="' . $item->node_code_path . $item->category_code . '/">';
                }

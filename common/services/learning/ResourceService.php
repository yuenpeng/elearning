...
//            $lnCoursesware->andWhere('kid in ('.$resourceResultSql.')');
        ...
        //demo 2张报表未配置
        $resource['lnReportCount'] = ($lnReportCount+1);
...
//            ->andFilterWhere(['=', 'is_display_pc', LnCourse::DISPLAY_PC_YES])
        ...
        if ($ids) {
            ...
            $categoryQuery = LnCourseCategory::find(false);

            $categoryQuery
                ->innerJoin(FwTreeNode::tableName() . ' tree', LnCourseCategory::tableName() . '.tree_node_id = tree.kid and tree.is_deleted = \'0\'')
                ->andFilterWhere([LnCourseCategory::tableName() . '.company_id' => $companyId])
                ->andFilterWhere(['tree.status' => FwTreeNode::STATUS_FLAG_NORMAL])
                ->select(LnCourseCategory::tableName() . '.kid');

            $categoryQuerySql = $categoryQuery->createCommand()->rawSql;
            $courseQuery->andWhere("category_id in ( $categoryQuerySql )");
        }
    ...
    public function getCourseResDetail($courseId, $userId)
    {
        ...
        foreach ($modRes as $resMode) {
            $modResId = $resMode['kid'];
            $mod_name = LnCourseMods::find()->where(['kid'=>$resMode['mod_id']])->one()->mod_name;
            $componentId = $resMode['component_id'];
            $componentModel = LnComponent::findOne($componentId);
            $componentCode = $componentModel->component_code;

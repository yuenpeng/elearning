...
//	}
	 ...
	 * @param string $userId
	 * @return multitype:multitype: NULL unknown \yii\caching\mixed
	 */
...
//			$question_type=$investigationQuestion->question_type;
		...
		foreach ($arrs as $arr) {
			...
			foreach ($arr as $k => $v) {
				...
				if ($k == "option_title") {
					...
					$LnInvestigationResult->$k = Html::encode($v);
				}else if ($k == "course_process_complete_id"){
	...
	*/
		...
		//刷新缓存
		$cacheKey = 'single-investigation-survey-query-'.$investigation_id;
		Yii::$app->cache->delete($cacheKey);
	...
	public function getVoteResultComplete($id, $courseId, $modResId, $type, $defaultPageSize = 10, $status = null, $keywords = null){
        ...
        /*关联课件完成表*/
        $model->leftJoin($resCompleteTableName, $resCompleteTableName.'.user_id='.$regTable.".user_id and ".$resCompleteTableName.".complete_type=".LnResComplete::COMPLETE_TYPE_FINAL." and ".$resCompleteTableName.".mod_res_id='".$modResId."' and ".$resCompleteTableName.".course_id=".$regTable.".course_id and ".$resCompleteTableName.".is_deleted=0");
        ...
        if ($count > 0){
            ...
            $result = $model->select([
                ...
                $resCompleteTableName.'.end_at'
            ])->asArray()->all();
            $vote = array();
            foreach ( $result as $k => $item){
                $vote[$k] = $item;
                if ($item['complete_status'] == LnResComplete::COMPLETE_STATUS_DONE) {
                    ...
                    if (!empty($investigationData)) {
                        $option = array();
                        foreach ($investigationData as $t) {
                            $votetemp = LnInvestigationOption::findOne($t->investigation_option_id);
                            if (!empty($votetemp)) {
                                $option[] = chr(64 + $votetemp['sequence_number']);
                            }
                        }
                        sort($option);
                        $option = array_unique($option);
                        $vote[$k]['option'] = join('、', $option);
                    } else {
                        $vote[$k]['option'] = "";
                    }
                } else {

...
class ResourceAudienceService extends LnResourceAudience
{
    ...
    public function isResourceAudience($userId, $resourceId){
        $resourceAudience = LnResourceAudience::find(false)->andFilterWhere(['resource_id' => $resourceId, 'status' => LnResourceAudience::STATUS_FLAG_NORMAL])->select("audience_id")->all();
        if (empty($resourceAudience)) return true;
        $audienceIdArray = TArrayHelper::map($resourceAudience, 'audience_id', 'audience_id');
        $audienceId = array_keys($audienceIdArray);
        $audienceService = new AudienceManageService();
        $user = $audienceService->getAudienceMember($audienceId);

...
class AudienceManageService extends SoAudience
{
    ...
    public function betchInsertAudeniceTemp($params){
        ...
        if (!empty($user)){
            ...
            foreach ($user as $v){
                $item = FwUser::findOne($v);
                $hasData = SoAudienceTemp::findOne(['audience_batch'=>$params['audience_batch'],'owner_id' => $ownerId,'user_id'=>$item->kid]);
                if (empty($hasData)) {
                    $model = new SoAudienceTemp();
                    $model->audience_batch = $params['audience_batch'];
                    $model->company_id = $companyId;
                    $model->owner_id = $ownerId;
                    $model->user_id = $item->kid;
                    $model->user_name = $item->user_name;
                    $model->real_name = $item->real_name;
                    $model->orgnization = $userService->getOrgnizationStringByUserId($item->kid);
                    $positionAll = $userService->getPositionListByUserId($item->kid);
                    if (!empty($positionAll)) {
                        $position_name = TArrayHelper::map($positionAll, 'kid', 'position_name');
                    ...
                    $model->status = SoAudienceTemp::STATUS_FLAG_NORMAL;
    ...
    public function betchImportAudeniceTemp($params){
        ...
        if (!empty($data['success'] )) {
            ...
            foreach ($data['success'] as $item) {
                if ($item['msg'] == 'success') {
                    $fwUser = $item['fwUser'];
                    $count = SoAudienceTemp::find(false)->andFilterWhere(['audience_batch' => $params['audience_batch'], 'owner_id' => $ownerId, 'user_id' => $fwUser->kid])->count('kid');
                    if (empty($count)) {
                        $model = new SoAudienceTemp();
                        $model->audience_batch = $params['audience_batch'];
                        $model->company_id = $companyId;
                        $model->owner_id = $ownerId;
                        $model->user_id = $fwUser->kid;
                        $model->user_name = $fwUser->user_name;
                        $model->real_name = $fwUser->real_name;
                        $model->orgnization = $userService->getOrgnizationStringByUserId($fwUser->kid);
                        $positionAll = $userService->getPositionListByUserId($fwUser->kid);
                        if (!empty($positionAll)) {
                            $position_name = TArrayHelper::map($positionAll, 'kid', 'position_name');
                            $position_name = join('、', $position_name);
                        }else{
    ...
    public function saveAudienceTemp($params){
        ...
        $user_id = $soAudienceTempModel->select('user_id')->orderBy('created_at desc')->asArray()->all();
            ...
            if (!empty($audienceId)) {
                ...
                foreach ($user_id as $item) {
                    ...
                    if (empty($modelTemp)){
                        ...
                        $modelTemp->status = SoAudienceMember::STATUS_FLAG_NORMAL;
                ...
                $errMsg = "";
                if (!empty($modelArray)) {
                    BaseActiveRecord::batchInsertSqlArray($modelArray, $errmsg);
                }
            }
    ...
    public function createAudienceTemp($audienceId, $audience_batch){
        ...
        foreach ($dataMemberAll as $item){
            $user = FwUser::findOne($item->user_id);
            $model = new SoAudienceTemp();
            ...
            $model->orgnization = $userService->getOrgnizationStringByUserId($item->user_id);
            $positionAll = $userService->getPositionListByUserId($item->user_id);
            if (!empty($positionAll)) {
                $position_name = TArrayHelper::map($positionAll, 'kid', 'position_name');
                $position_name = join('、', $position_name);
            }else{
                ...
                if (!empty($sheet_0)) {
                    ...
                    foreach ($sheet_0 as $i => $item) {
                        if ($i > 0) {
                            ...
                            $findUser = FwUser::findOne(['user_name' => $userName,'company_id' => $companyId]);
                            if (empty($findUser)) {

...
class BoeTxyWeilogService {
    ...
    private static $cacheNameFix = 'boe_txy_weilog_';
        ...
        if ($deleteValue < 1) {
            switch ($deleteValue) {
                ...
                case -4:
                    ...
                    break;
                case -5:
                    $sult['errorCode'] = Yii::t('boe', 'boe_txy_welog_send_error');
                    break;
                case -6:
                    $sult['errorCode'] = Yii::t('boe', 'boe_txy_weilog_time_error');
                    break;
            }
        }
        return $sult;
                ...
                if ($log_id) {
                    ...
                    if ($find) {
                        ...
                        if (isset($find_num) && $find_num >= 1) {
                            ...
                            if ($recommendValue) {
                                $sult['recommendText'] = $data['recommend_status'] == 1 ? Yii::t('boe', 'boe_txy_weilog_recommend_yes') : Yii::t('boe', 'boe_txy_weilog_recommend_no');
				...
				if ($log_id) {
					...
					if ($find) {
                        ...
                        if (isset($find_num) && $find_num > 2) {
							...
							if ($publishValue) {
								$sult['publishText'] = $data['publish_status'] == 1 ? Yii::t('boe', 'boe_txy_weilog_publish_yes') : Yii::t('boe', 'boe_txy_weilog_publish_no');
								$sult['publishClass'] = $data['publish_status'] == 1 ? "btn-default" : "btn-info";
								$sult['publishText2'] = $data['publish_status'] == 1 ? Yii::t('boe', 'boe_txy_weilog_publish_yes2') : Yii::t('boe', 'boe_txy_weilog_publish_no2');
    ...
    public static function getTeacherTodayLogGroup($orgnization_id) {
        ...
        if ($orgnization) {
            ...
            $field = array(
                'eln_boe_txy_weilog.created_by as kid',
                'eln_fw_user.real_name as real_name',
                new \yii\db\Expression('count(*) as num'),
            );
			$current_time 		= time();//当前时间
			$today_time 		= strtotime(date("Y-m-d"));//今天
			$where_time			= array('>=', 'eln_boe_txy_weilog.created_at', $today_time);
            $where = array(
                'and',
                array('=', 'eln_boe_txy_weilog.orgnization_id', $orgnization_id),
                array('=', 'eln_boe_txy_weilog.is_deleted', 0),
                array('=', 'eln_fw_user.is_deleted', 0),
                array('<>', 'eln_fw_user.status', FwUser::STATUS_FLAG_STOP),
                $where_time,	
            );
            $query = new Query();
            $query->from('eln_boe_txy_weilog')->select($field)->join('INNER JOIN', 'eln_fw_user', "eln_fw_user.kid=eln_boe_txy_weilog.created_by");
            $query->where($where)->groupBy('eln_boe_txy_weilog.created_by')->orderBy('num desc')->indexBy('kid');
            $tmp_sult = array(
    ...
    public static function getLogList($params = array()) {
        ...
        $where = array(
            'and',
            //array('in', 'eln_boe_txy_weilog.orgnization_id', $params['orgnization_id']),
            //array('=', 'eln_boe_txy_weilog.is_deleted', 0),
        );
        if (isset($params['time']) && $params['time']) {
            ...
            switch ($params['time']) {
                case 'today'://今天发布的
					$where[] = array('>=', 'eln_boe_txy_weilog.created_at', $today_time);
                    break;
                case 'yesterday'://昨天发布的
                    $where[] = array('between', 'eln_boe_txy_weilog.created_at', $yesterday_time, $today_time - 1);
                    break;
                case 'other_day'://两天前发布的
                    $where[] = array('<', 'eln_boe_txy_weilog.created_at', $yesterday_time - 1);
                    break;
            }
        }
        //组织
        if (isset($params['orgnization']) && $params['orgnization']) {
            $where[] = array(is_array($params['orgnization']) ? 'in' : '=', 'eln_boe_txy_weilog.orgnization_id', $params['orgnization']);
        }
        //用户
        if (isset($params['user_id']) && $params['user_id']) {
            $where[] = array(is_array($params['user_id']) ? 'in' : '=', 'eln_boe_txy_weilog.created_by', $params['user_id']);
        }
        //推荐
        if (isset($params['recommend_status']) && $params['recommend_status'] && ($params['recommend_status'] == '0' || $params['recommend_status'] == '1')) {
            $where[] = array('=', 'eln_boe_txy_weilog.recommend_status', $params['recommend_status']);
        }
        //发布
        if (isset($params['publish_status']) && $params['publish_status'] && ($params['publish_status'] == '0' || $params['publish_status'] == '1')) {
            $where[] = array('=', 'eln_boe_txy_weilog.publish_status', $params['publish_status']);
        }
		//return $where;
        $db_obj = new BoeTxyWeilog();
        $p = array(
        ...
        if (isset($params['get_orgnization_path']) && $params['get_orgnization_path'] == 1 && isset($sult['list']) && is_array($sult['list'])) {
            ...
            foreach ($sult['list'] as $s_key => $s_value) {
                ...
                $sult['list'][$s_key]['recommend_text'] = $s_value['recommend_status'] == 1 ? Yii::t('boe', 'boe_txy_weilog_recommend_yes') : Yii::t('boe', 'boe_txy_weilog_recommend_no');
                $sult['list'][$s_key]['recommend_class'] = $s_value['recommend_status'] == 1 ? "red_star" : "no_star";
                $sult['list'][$s_key]['publish_text'] = $s_value['publish_status'] == 1 ? Yii::t('boe', 'boe_txy_weilog_publish_yes') : Yii::t('boe', 'boe_txy_weilog_publish_no');
                $sult['list'][$s_key]['publish_class'] = $s_value['publish_status'] == 1 ? "btn-default" : "btn-info";

                $sult['list'][$s_key]['publish_text2'] = $s_value['publish_status'] == 1 ? Yii::t('boe', 'boe_txy_weilog_publish_yes2') : Yii::t('boe', 'boe_txy_weilog_publish_no2');
            ...
            foreach ($sult['user_data'] as $s_key => $_value) {
                $orgnization_path = BoeTxyService::boeTrim($_value['orgnization_path'], "2017特训营\\");
	...
	*/
	public static function submitWeiog($params = array()) {
		...
		if(!isset($params['user_id']) || !$params['user_id'])
		{
			$sult = array(
			   'error' => -7,
			   'message' => Yii::t('boe', 'boe_txy_weilog_user_id_must')
			);
			return $sult;	
		}
		//缓存名称
		...
		if(!isset($params['content']) || !$params['content'])
		{
			$sult = array(
			   'error' => -6,
			   'message' => Yii::t('boe', 'boe_txy_weilog_content_must')
			);
			return $sult;	
		}
		/*if(strlen($params['content'])<300)
		{
			$sult = array(
			   'error' => -5,
			   'message' => Yii::t('boe', 'boe_txy_weilog_content_len')
			);
			return $sult;
		}*/
		...
		}else
		{
			...
			if($sult_cache == 1)
			{
				$sult = array(
					'error' => -3,
					'message' => Yii::t('boe', 'boe_txy_weilog_today_added')
				);
				return $sult;
			}
			//判断今天是都已发表过日志信息
		...
		if (is_array($db_sult) || $db_sult === false) {
			if (is_array($db_sult)) {
				...
				$sult['error'] = -2;
				$sult['message'] = Yii::t('boe', 'boe_txy_weilog_save_error');
			}
		}else
		{
			if(!isset($params['log_id'])||!$params['log_id'])

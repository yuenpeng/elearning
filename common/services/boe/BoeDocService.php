        ...
        foreach ($id_array as $a_file_id) {//拼凑出要删除的文件和云盘上要删除的KEY开始
            ...
            if ($tmp_check) {//文件检测通过S
                ...
                if ($physicalDelete) {//是否进行物理删除S
                    ...
                    $need_mp4 = BoeDocFile::checkFileCanConvertMp4($file_ext) || $file_ext == 'mp4';
                    ...
                    if ($cloud_status == 3) {//已经上传到云了S
                        ...
                        if ($need_mp4) {
                            ...
                            $ks3_file_info[$bucket][] = str_replace(".{$file_ext}", BoeDocFile::getMp4ConvertLastString() . '.mp4', $file_key);
                        }
                        if ($need_mp3) {
            ...
            case 3://上传成功 S
                ...
                if (!$source_mode) {//不是获取原始文件的状态,根据扩展名获取相应的转码后的文件S
                    if (BoeDocFile::checkFileCanConvertPdf($file_ext)) {
                        $last_file_type = 'pdf';
                    } elseif (BoeDocFile::checkFileCanConvertMp4($file_ext) || $file_ext == 'mp4') {
                        $last_file_type = 'mp4';
                ...
                if ($get_adp) {//对于那些个需要获取转换后的文件代码S
                    ...
                    if ($convert_status == 1) {//转码完成了，并且回调已经响应了S
                        ...
                        if ($process_status != 3) {//转码未完成
                            ...
                            if ($file_ext == 'mp4') {
                                $file_key = str_replace(".{$file_ext}", BoeDocFile::getMp4ConvertLastString() . ".{$last_file_type}", $file_key);
                            } else {
                                 $file_key = str_replace(".{$file_ext}", ".{$last_file_type}", $file_key);
                            }
...
//                if ($key == '658BEDBA-35BA-EBE8-6A32-B76E8FD2CD14') {
//                    BoeBase::debug(BoeDocFile::getMp4ConvertLastString());
//                    BoeBase::debug($file_key);
//                    BoeBase::debug($file_info, 1);
//                }
    ...
    static function redirectFile($file_info = array(), $file_url = NULL, $is_down_mode = 0) {
        ...
        if (!is_numeric($file_url)) {
            ...
            $encoded_filename = rawurlencode ( $file_info['old_file_name'] );
            if (preg_match ( "/MSIE/", $broswer )) {
                header ( 'Content-Disposition: attachment; filename="' . $encoded_filename . '"' );
            } else if (preg_match ( "/Firefox/", $broswer )) {

...
use yii\db\Query;
use Yii;
use common\models\boe\BoeGrowUser;
 ...
 * Date: 2017/9/1
 */
class BoeGrowService extends FwUser{
    ...
    public function saveImport($data, $file, $fileMd5)
    {
        ...
        foreach ($data as $index => $item) {
            if ($item['op'] === 'A') {
                ...
                $model->step3_score = $item['step3_score'];
                $model->step4_score = $item['step4_score'];
                $model->organizer = $item['organizer'];
                $sheet->setCellValue($errColum . ($index + 1), 'success');
                $saveList[] = $model;
            } elseif ($item['op'] === 'U') {
                $model = BoeGrowUser::findOne(['user_no' => $item['user_no'],'is_deleted'=>0]);
                if ($model) {
                    $model->year = $item['year'];
                    $model->id_number = $item['id_number'];
                    $model->real_name = $item['real_name'];
                    if($item['step1_score']!='-1'){
                        $model->step1_score = $item['step1_score'];
                    }
                    if($item['step2_score']!='-1'){
                        $model->step2_score = $item['step2_score'];
                    }
                    if($item['step3_score']!='-1'){
                        $model->step3_score = $item['step3_score'];
                    }
                    if($item['step4_score']!='-1'){
                        $model->step4_score = $item['step4_score'];
                    }
                    $model->organizer = $item['organizer'];
                    $sheet->setCellValue($errColum . ($index + 1), 'success');
                    ...
                    $sheet->setCellValue($errColum . ($index + 1), 'user not found');
                }
            } elseif ($item['op'] === 'D') {
                $model = BoeGrowUser::findOne(['user_no' => $item['user_no'],'is_deleted'=>0]);
                if ($model) {
    ...
    public function search($params, $managerFlag, $parentNodeId, $includeSubNode)
    {
        ...
        $query
            ->leftJoin(FwOrgnization::realTableName(),
                BoeGrowUser::tableName() . "." . BaseActiveRecord::getQuoteColumnName("orgnization_id") . " = " . FwOrgnization::tableName() . "." . BaseActiveRecord::getQuoteColumnName("kid"))
...
//            ->innerJoinWith('orgnization.treeNode')
            ...
            ->andFilterWhere(['=', 'user_no', trim(urldecode($this->user_no))]);
        ...
        if (!Yii::$app->user->getIsGuest()) {
            ...
            if (!$isSpecialUser) {
                ...
                if (!empty($userOrgnizations)) {
                    ...
                    if ($isSysManager) {
                        $query->andFilterWhere(['=', BoeGrowUser::realTableName() . '.company_id', $companyId]);
                    }
                }
            }
            ...
            if (isset($selectedResult) && $selectedResult != null) {
                ...
                $query->andFilterWhere(['in', BoeGrowUser::realTableName() . '.orgnization_id', $orgnizationIdList]);
            } else {

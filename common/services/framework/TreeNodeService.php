...
use common\services\boe\BoeMixtureCourseGroupCategoryService;
use common\services\boe\BoeMixtureCourseProjectCategoryService;
use common\services\boe\BoeMixtureCourseProjectService;
use common\services\boe\MixtureCourseGroupCategoryService;
use common\services\boe\MixtureCourseProjectCategoryService;
...
//            return listAllTreeNodes();
        ...
        if (!Yii::$app->user->getIsGuest()) {
            ...
            }elseif ($treeType == 'course-project-category'){
                $userCompanyService = new UserCompanyService();
                $selectedCompanyResult = $userCompanyService->getManagedListByUserId($userId,null,$withSession);

                $selectedCompanyList = ArrayHelper::map($selectedCompanyResult, 'kid', 'kid');

                $MixtureCourseProjectCategoryService = new BoeMixtureCourseProjectCategoryService();
                $selectedResult = $MixtureCourseProjectCategoryService->GetProjectCategoryByCompanyIdList($selectedCompanyList);
            }elseif($treeType == 'course-group-category'){
                $userCompanyService = new UserCompanyService();
                $selectedCompanyResult = $userCompanyService->getManagedListByUserId($userId,null,$withSession);

                $selectedCompanyList = ArrayHelper::map($selectedCompanyResult, 'kid', 'kid');

                $MixtureCourseProjectCategoryService = new BoeMixtureCourseGroupCategoryService();
                $selectedResult = $MixtureCourseProjectCategoryService->GetGroupCategoryByCompanyIdList($selectedCompanyList);
            }
...
//                $subChildren =  $subNodeList->count(1) > 0 ? true : false;
                ...
                if ($showContentCount) {
                    if ($treeType == "course-category") {
                        ...
                        $treeNodeName .= "(" . strval($count) . ")";
                    }else if($treeType == "course-group-category"){
                        $groupCategoryService = new BoeMixtureCourseGroupCategoryService();
                        $count = $groupCategoryService->GetGroupCategoryCount($treeNodeKid, $ListRouteParams);
                        $treeNodeName .= "(" . strval($count) . ")";
                    }else if($treeType = "course-project-category"){
                        $objectCategoryService = new BoeMixtureCourseProjectCategoryService();
                        $count = $objectCategoryService->GetProjectCategoryCount($treeNodeKid,$ListRouteParams);
                        $treeNodeName .= "(" . strval($count) . ")";
                    }
...
//        $sourceParentModel = TreeNode::findOne($sourceParentNodeId);
        ...
        $attributes = [
            ...
            'node_name_path' => new Expression("concat(:newNodeNamePath,substring(" . BaseActiveRecord::getQuoteColumnName("node_name_path") .",locate(:oldNodeNamePath, " . BaseActiveRecord::getQuoteColumnName("node_name_path") .")+CHAR_LENGTH(:oldNodeNamePath)))",
    ...
    //更新子节点数据
    public function updateChildTreeNode($kid, $treeNodeCode){
        $find = FwTreeNode::findOne(['kid' => $kid, 'tree_node_code' => $treeNodeCode]);
        if(empty($find)) return false;
        $list = FwTreeNode::findAll(['parent_node_id' => $kid, 'tree_type_id' => $find->tree_type_id]);
        if (!empty($list)) {
            //更新子节点
            FwTreeNode::updateAll(
                [
                    'node_id_path' => $find->node_id_path . $find->kid . '/',
                    'node_name_path' => $find->node_name_path.$find->tree_node_name.'/',
                    'node_code_path' => $find->node_code_path.$find->tree_node_code.'/',
                    'root_node_id' => $find->root_node_id,
                    'tree_level' => $find->tree_level + 1,
                ],
                "parent_node_id=:parent_node_id and tree_type_id=:tree_type_id",
                [
                    ':parent_node_id' => $kid,
                    ':tree_type_id' => $find->tree_type_id
                ]
            );
            foreach ($list as $val) {
                $this->updateChildTreeNode($val->kid, $val->tree_node_code);
            }
        }else{
            return true;
        }

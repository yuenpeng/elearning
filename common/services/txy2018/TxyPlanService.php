...
class TxyPlanService {
    ...
    private static $cacheNameFix = 'boe_txy2018_plan_';
        ...
        if ($deleteValue < 1) {
            switch ($deleteValue) {
                ...
                case -4:
                    ...
                    break;
                case -5:
                    $sult['errorCode'] = Yii::t('txy', 'txy_plan_send_error');
                    break;
                case -6:
                    $sult['errorCode'] = Yii::t('txy', 'txy_plan_time_error');
                    break;
            }
        }
        return $sult;
                ...
                if ($plan_id) {
                    ...
                    if ($find) {
                        ...
                        //} else {
                            ...
                            if ($recommendValue) {
                                $sult['recommendText'] = $data['recommend_status'] == 1 ? Yii::t('txy', 'txy_plan_recommend_yes') : Yii::t('txy', 'txy_plan_recommend_no');
				...
				if ($plan_id) {
					...
					if ($find) {
                        ...
                        //} else {
							...
							if ($publishValue) {
								$sult['publishText'] = $data['publish_status'] == 1 ? Yii::t('txy', 'txy_plan_publish_yes') : Yii::t('txy', 'txy_plan_publish_no');
								$sult['publishClass'] = $data['publish_status'] == 1 ? "btn-default" : "btn-info";
								$sult['publishText2'] = $data['publish_status'] == 1 ? Yii::t('txy', 'txy_plan_publish_yes2') : Yii::t('txy', 'txy_plan_publish_no2');
    ...
    public static function getTeacherTodayPlanGroup($orgnization_id) {
        ...
        //if ($orgnization) {
            ...
            $field = array(
                'eln_txy2018_plan.created_by as kid',
                'eln_fw_user.real_name as real_name',
                new \yii\db\Expression('count(*) as num'),
            );
			//$current_time 		= time();//当前时间
			//$today_time 		= strtotime(date("Y-m-d"));//今天
			//$where_time			= array('>=', 'eln_txy2018_plan.created_at', $today_time);
            $where = array(
                'and',
                array('=', 'eln_txy2018_plan.orgnization_id', $orgnization_id),
                array('=', 'eln_txy2018_plan.is_deleted', 0),
                array('=', 'eln_fw_user.is_deleted', 0),
                array('<>', 'eln_fw_user.status', FwUser::STATUS_FLAG_STOP),
                //$where_time,
            );
            $query = new Query();
            $query->from('eln_txy2018_plan')->select($field)->join('INNER JOIN', 'eln_fw_user', "eln_fw_user.kid=eln_txy2018_plan.created_by");
            $query->where($where)->groupBy('eln_txy2018_plan.created_by')->orderBy('num desc')->indexBy('kid');
            $tmp_sult = array(
    ...
    public static function getPlanList($params = array()) {
        ...
        $where = array(
            'and',
            //array('in', 'eln_txy2018_plan.orgnization_id', $params['orgnization_id']),
            //array('=', 'eln_txy2018_plan.is_deleted', 0),
        );
        if (isset($params['time']) && $params['time']) {
            ...
            switch ($params['time']) {
                case 'today'://今天发布的
					$where[] = array('>=', 'eln_txy2018_plan.created_at', $today_time);
                    break;
                case 'yesterday'://昨天发布的
                    $where[] = array('between', 'eln_txy2018_plan.created_at', $yesterday_time, $today_time - 1);
                    break;
                case 'other_day'://两天前发布的
                    $where[] = array('<', 'eln_txy2018_plan.created_at', $yesterday_time - 1);
                    break;
            }
        }
        //组织
        if (isset($params['orgnization']) && $params['orgnization']) {
            $where[] = array(is_array($params['orgnization']) ? 'in' : '=', 'eln_txy2018_plan.orgnization_id', $params['orgnization']);
        }
        //用户
        if (isset($params['user_id']) && $params['user_id']) {
            $where[] = array(is_array($params['user_id']) ? 'in' : '=', 'eln_txy2018_plan.created_by', $params['user_id']);
        }
        //推荐
        if (isset($params['recommend_status']) && $params['recommend_status'] && ($params['recommend_status'] == '0' || $params['recommend_status'] == '1')) {
            $where[] = array('=', 'eln_txy2018_plan.recommend_status', $params['recommend_status']);
        }
        //发布
        if (isset($params['publish_status']) && $params['publish_status'] && ($params['publish_status'] == '0' || $params['publish_status'] == '1')) {
            $where[] = array('=', 'eln_txy2018_plan.publish_status', $params['publish_status']);
        }
        $db_obj = new Txy2018Plan();
        $p = array(
        ...
        if (isset($params['get_orgnization_path']) && $params['get_orgnization_path'] == 1 && isset($sult['list']) && is_array($sult['list'])) {
            ...
            foreach ($sult['list'] as $s_key => $s_value) {
                ...
                $sult['list'][$s_key]['recommend_text'] = $s_value['recommend_status'] == 1 ? Yii::t('txy', 'txy_plan_recommend_yes') : Yii::t('txy', 'txy_plan_recommend_no');
                $sult['list'][$s_key]['recommend_class'] = $s_value['recommend_status'] == 1 ? "red_star" : "no_star";
                $sult['list'][$s_key]['publish_text'] = $s_value['publish_status'] == 1 ? Yii::t('txy', 'txy_plan_publish_yes') : Yii::t('txy', 'txy_plan_publish_no');
                $sult['list'][$s_key]['publish_class'] = $s_value['publish_status'] == 1 ? "btn-default" : "btn-info";

                $sult['list'][$s_key]['publish_text2'] = $s_value['publish_status'] == 1 ? Yii::t('txy', 'txy_plan_publish_yes2') : Yii::t('txy', 'txy_plan_publish_no2');
	...
	*/
	public static function submitPlan($params = array()) {
		...
		if(!isset($params['user_id']) || !$params['user_id'])
		{
			$sult = array(
			   'error' => -7,
			   'message' => Yii::t('txy', 'txy_plan_user_id_must')
			);
			return $sult;	
		}
		//缓存名称
		$cache_name = __METHOD__ . '_user_id_' . $params['user_id'];
		//return $cache_name;
		if(!isset($params['content']) || !$params['content'])
		{
			$sult = array(
			   'error' => -6,
			   'message' => Yii::t('txy', 'txy_plan_content_must')
			);
			return $sult;	
		}
		...
		}else
		{
			...
			if($sult_cache == 1)
			{
				$sult = array(
					'error' => -3,
					'message' => Yii::t('txy', 'txy_plan_today_added')
				);
				return $sult;
			}
			//判断今天是都已发表过日志信息
			...
			if($check_sult['totalCount'] > 0 )
			{
				$sult = array(
					'error' => -3,
					'message' => Yii::t('txy', 'txy_plan_added')
				);
				//写入缓存
		...
		if (is_array($db_sult) || $db_sult === false) {
			if (is_array($db_sult)) {
				...
				$sult['error'] = -2;
				$sult['message'] = Yii::t('txy', 'txy_plan_save_error');
			}
		}else
		{
			if(!isset($params['plan_id'])||!$params['plan_id'])

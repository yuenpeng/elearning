...
class TxyWeilogService {
    ...
    private static $cacheNameFix = 'txy2018_weilog_';
        ...
        if ($deleteValue < 1) {
            switch ($deleteValue) {
                ...
                case -4:
                    ...
                    break;
                case -5:
                    $sult['errorCode'] = Yii::t('txy', 'txy_welog_send_error');
                    break;
                case -6:
                    $sult['errorCode'] = Yii::t('txy', 'txy_weilog_time_error');
                    break;
            }
        }
        return $sult;
    ...
    public static function updateLogRecommendStatus($log_id, $user_id, $c_key) {
        $recommendValue = 0;
        $log_data = array();
        $sult = array(
            'recommendValue' => 0,
        ...
        if (!$user_id) {
            ...
            $level = TxyService::getUserLevel2($user_id);
			//return $level;
            if ($level[$c_key]['level'] != 1) {
                $recommendValue = -10; //无推荐权限
            } else {
                $orgnization_id	= $level[$c_key]['info']['orgnization_id'];
				$manager_area = TxyService::getManagerArea($orgnization_id, 0);
				//return $manager_area;
                if ($log_id) {
                    $logObj = new Txy2018Weilog();
                    $where = array('kid' => $log_id);
                    ...
                    if ($find) {
                        ...
                        //} else {
                            ...
                            if ($recommendValue) {
                                $sult['recommendText'] = $data['recommend_status'] == 1 ? Yii::t('txy', 'txy_weilog_recommend_yes') : Yii::t('txy', 'txy_weilog_recommend_no');
    ...
    public static function updateLogPublishStatus($log_id, $user_id, $c_key) {
        $publishValue = 0;
        $log_data = array();
        $sult = array(
            'publishValue' => 0,
				...
				if ($log_id) {
					...
					if ($find) {
                        ...
                        //} else {
							...
							if ($publishValue) {
								$sult['publishText'] = $data['publish_status'] == 1 ? Yii::t('txy', 'txy_weilog_publish_yes') : Yii::t('txy', 'txy_weilog_publish_no');
								$sult['publishClass'] = $data['publish_status'] == 1 ? "btn-default" : "btn-info";
								$sult['publishText2'] = $data['publish_status'] == 1 ? Yii::t('txy', 'txy_weilog_publish_yes2') : Yii::t('txy', 'txy_weilog_publish_no2');
     ...
     */
    public static function getTeacherTodayLogGroup($orgnization_id) {
        if (!$orgnization_id) {
            return NULL;
        }
        $orgnization = TxyService::getManagerArea($orgnization_id); //获取当前辅导员的辖区ID
		$sult = array();
        if ($orgnization) {
            $orgnization_id = $orgnization['kid'];
            $field = array(
                'eln_txy2018_weilog.created_by as kid',
                'eln_fw_user.real_name as real_name',
                new \yii\db\Expression('count(*) as num'),
            );
			$current_time 		= time();//当前时间
			$today_time 		= strtotime(date("Y-m-d"));//今天
			$where_time			= array('>=', 'eln_txy2018_weilog.created_at', $today_time);
            $where = array(
                'and',
                array('=', 'eln_txy2018_weilog.orgnization_id', $orgnization_id),
                array('=', 'eln_txy2018_weilog.is_deleted', 0),
                array('=', 'eln_fw_user.is_deleted', 0),
                array('<>', 'eln_fw_user.status', FwUser::STATUS_FLAG_STOP),
                $where_time,	
            );
            $query = new Query();
            $query->from('eln_txy2018_weilog')->select($field)->join('INNER JOIN', 'eln_fw_user', "eln_fw_user.kid=eln_txy2018_weilog.created_by");
            $query->where($where)->groupBy('eln_txy2018_weilog.created_by')->orderBy('num desc')->indexBy('kid');
            $tmp_sult = array(
    ...
    public static function getLogList($params = array()) {
        ...
        $where = array(
            'and',
            //array('in', 'eln_txy2018_weilog.orgnization_id', $params['orgnization_id']),
            //array('=', 'eln_txy2018_weilog.is_deleted', 0),
        );
        if (isset($params['time']) && $params['time']) {
            ...
            switch ($params['time']) {
                case 'today'://今天发布的
					$where[] = array('>=', 'eln_txy2018_weilog.created_at', $today_time);
                    break;
                case 'yesterday'://昨天发布的
                    $where[] = array('between', 'eln_txy2018_weilog.created_at', $yesterday_time, $today_time - 1);
                    break;
                case 'other_day'://两天前发布的
                    $where[] = array('<', 'eln_txy2018_weilog.created_at', $yesterday_time - 1);
                    break;
            }
        }
        //组织
        if (isset($params['orgnization']) && $params['orgnization']) {
            $where[] = array(is_array($params['orgnization']) ? 'in' : '=', 'eln_txy2018_weilog.orgnization_id', $params['orgnization']);
        }
        //用户
        if (isset($params['user_id']) && $params['user_id']) {
            $where[] = array(is_array($params['user_id']) ? 'in' : '=', 'eln_txy2018_weilog.created_by', $params['user_id']);
        }
        //推荐
        if (isset($params['recommend_status']) && $params['recommend_status'] && ($params['recommend_status'] == '0' || $params['recommend_status'] == '1')) {
            $where[] = array('=', 'eln_txy2018_weilog.recommend_status', $params['recommend_status']);
        }
        //发布
        if (isset($params['publish_status']) && $params['publish_status'] && ($params['publish_status'] == '0' || $params['publish_status'] == '1')) {
            $where[] = array('=', 'eln_txy2018_weilog.publish_status', $params['publish_status']);
        }
		//return $where;
        $db_obj = new Txy2018Weilog();
        $p = array(
        ...
        if (isset($params['get_orgnization_path']) && $params['get_orgnization_path'] == 1 && isset($sult['list']) && is_array($sult['list'])) {
            ...
            foreach ($sult['list'] as $s_key => $s_value) {
                ...
                $sult['list'][$s_key]['recommend_text'] = $s_value['recommend_status'] == 1 ? Yii::t('txy', 'txy_weilog_recommend_yes') : Yii::t('txy', 'txy_weilog_recommend_no');
                $sult['list'][$s_key]['recommend_class'] = $s_value['recommend_status'] == 1 ? "red_star" : "no_star";
                $sult['list'][$s_key]['publish_text'] = $s_value['publish_status'] == 1 ? Yii::t('txy', 'txy_weilog_publish_yes') : Yii::t('txy', 'txy_weilog_publish_no');
                $sult['list'][$s_key]['publish_class'] = $s_value['publish_status'] == 1 ? "btn-default" : "btn-info";

                $sult['list'][$s_key]['publish_text2'] = $s_value['publish_status'] == 1 ? Yii::t('txy', 'txy_weilog_publish_yes2') : Yii::t('txy', 'txy_weilog_publish_no2');
            ...
            foreach ($sult['user_data'] as $s_key => $_value) {
                $orgnization_path = TxyService::boeTrim($_value['orgnization_path'], "2018特训营\\");
	 ...
	 * 学员提交微日志信息
	*/
	public static function submitWeiog($params = array()) {
		...
		if(!isset($params['user_id']) || !$params['user_id'])
		{
			$sult = array(
			   'error' => -7,
			   'message' => Yii::t('txy', 'txy_weilog_user_id_must')
			);
			return $sult;	
		}
		//缓存名称
		$cache_name = __METHOD__ . '_user_id_' . $params['user_id'] . '_current_date_' .$params['created_at'];
		//return $cache_name;
		if(!isset($params['content']) || !$params['content'])
		{
			$sult = array(
			   'error' => -6,
			   'message' => Yii::t('txy', 'txy_weilog_content_must')
			);
			return $sult;	
		}
		/*if(strlen($params['content'])<300)
		{
			$sult = array(
			   'error' => -5,
			   'message' => Yii::t('txy', 'txy_weilog_content_len')
			);
			return $sult;
		}*/
		...
		$badword_check = TxyService::check_bad_word($data['content']);
		...
		}else
		{
			...
			if($sult_cache == 1)
			{
				$sult = array(
					'error' => -3,
					'message' => Yii::t('txy', 'txy_weilog_today_added')
				);
				return $sult;
			}
			//判断今天是都已发表过日志信息
			
			$start_at 	= strtotime($params['created_at']);
			$end_at 	= strtotime(date("Y-m-d",strtotime("+1 day",$start_at)));
			$params_where				= array(
				'and',
				array('=','user_id',$params['user_id']),
				array('>=','created_at',$start_at),
				array('<=','created_at',$end_at),
				array('=','is_deleted','0'),
			);
			$check_count = Txy2018Weilog::find(false)->where($params_where)->count();
			//$check_sult = self::getLogList($params_log);
			if($check_count > 0 )
			{
				$sult = array(
					'error' => -3,
		...
		if (is_array($db_sult) || $db_sult === false) {
			if (is_array($db_sult)) {
				...
				$sult['error'] = -2;
				$sult['message'] = Yii::t('txy', 'txy_weilog_save_error');
			}
		}else
		{
			if(!isset($params['log_id'])||!$params['log_id'])
			...
			//积分
			//TxyApiService::addWeiLogIntegral($this->current_user_id);
		}
		$dbObj = NULL;
		return $sult;	
	}
	 ...
	 * 学员提交微日志信息[补交信息]
	*/
	public static function submitWeiog2($params = array()) {
		...
		if(!isset($params['created_at']) || !$params['created_at'])
		{
			$sult = array(
			   'error' => -8,
			   'message' => Yii::t('txy', 'txy_created_at_must')
			);
			return $sult;	
		}
		//$current_date	= $params['created_at'];
		//缓存名称
		//$cache_name = __METHOD__ . '_user_id_' . $params['user_id'] . '_current_date_' .$current_date;
		//return $cache_name;
		if(!isset($params['content']) || !$params['content'])
		{
			$sult = array(

...
class RightInterface
{
    ...
    public function getManagedCompanyListByUserId($userId, $status = BaseActiveRecord::STATUS_FLAG_NORMAL, $withSession = true){
        $data = $this->getUserCompanyService()->getManagedListByUserId($userId, $status, $withSession);
        $model = FwCompany::find(false);
        return $data ? $model->populate($data) : $data;
    }
    ...
    public function getSearchCompanyListByUserId($userId, $status = BaseActiveRecord::STATUS_FLAG_NORMAL, $withSession = true){
        $data = $this->getUserCompanyService()->getSearchListByUserId($userId, $status, $withSession);
        $model = FwCompany::find(false);
        return $data ? $model->populate($data) : $data;
    }
    ...
    public function getManagedDomainListByUserId($userId, $status = BaseActiveRecord::STATUS_FLAG_NORMAL, $withSession = true){
        $data = $this->getUserDomainService()->getManagedListByUserId($userId, $status, $withSession);
    ...
    public function getSearchDomainListByUserId($userId, $status = BaseActiveRecord::STATUS_FLAG_NORMAL, $withSession = true){
        $data = $this->getUserDomainService()->getSearchListByUserId($userId, $status, $withSession);
        $model = FwDomain::find(false);
        return $data ? $model->populate($data) : $data;
    }
    ...
    public function getRoleListByUserId($userId, $withSession = true){
        $data = $this->getUserService()->getRoleListByUserId($userId, $withSession);
        $model = FwUserRole::find(false);
        return $data ? $model->populate($data) : $data;
    }
     ...
     * @param $companyId
     * @param $menuType
     * @return array|FwCompanyMenu[]
     */
    public function getCompanyMenuByType($companyId, $menuType){
        $data = $this->getCompanyMenuService()->getCompanyMenuByType($companyId, $menuType);
        $model = FwCompanyMenu::find(false);
        return $data ? $model->populate($data) : $data;
    }
}
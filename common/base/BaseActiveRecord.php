...
class BaseActiveRecord extends ActiveRecord
{
    ...
    public static $saveUpdateVersion = true;//保存数据前，更新版本号

    public function init()
    {
        self::$saveUpdateVersion = true;
        self::$defaultKey = "PC";
    }

    private static function getForcePhysicalDeleteTable() {
        $forcePhysicalDeleteTable = [
            '{{%ln_scorm_scoes_track}}',
            '{{%fw_action_log}}',
            '{{%fw_service_log}}'
        ];

        return $forcePhysicalDeleteTable;
    }

    private static function getForceCacheTable() {
    ...
    public static function find($includeDeleted = true)
    {
        ...
        $forcePhysicalDeleteTable = self::getForcePhysicalDeleteTable();
        if (!array_key_exists($tableName, $forcePhysicalDeleteTable)) {
            if (!$includeDeleted) {
...
//        $duration = 0;
        $tableName = parent::tableName();
        $forceCacheTable = self::getForceCacheTable();
        if (count($forceCacheTable) > 0) {
            if (array_key_exists($tableName, $forceCacheTable)) {
                $depandMode = $forceCacheTable[$tableName]["depend"];
                $duration = $forceCacheTable[$tableName]["duration"];
                $withCache = true;
            }
        }
        ...
        if ($withCache) {
            ...
            if ($isSingleKey) {
                ...
                $result = self::loadFromCache($cacheKey, true, $hasCache, true);
            }
            else {
                $result = null;
                $key = null;
            ...
            if ($result == false || empty($result)) {
                ...
                if ($isSingleKey && !empty($result)) {
                    if ($depandMode == self::DEPEND_MODE_SQL) {
                        ...
                        self::saveToCache($cacheKey, $result, $dependencySql, $duration, $withCache, true);
                    }
                    else {
                        self::saveToCache($cacheKey, $result, null, $duration, $withCache, true);
                    }
                }
            }
        }
...
//                $kids = "'{$kids}'";
                                     ...
                                     $saveEncode = false, $systemKey = null, $withVersion = true)
    {
        ...
        if ($withVersion && self::$saveUpdateVersion) {
            $newAttributes = [
...
//            $condition = 'kid in (' . $kids . ') AND is_deleted="' . self::DELETE_FLAG_NO . '"';
     ...
     * @param bool|true $saveUpdateVersion
     */
    public static function prepareData($model,$currentDate = null,$currentUserId = null, $systemKey = null, $insert = true, $saveUpdateVersion = true,$checkKeyExist = false,$needReturnKey = true,&$errMsg = null,&$resultId = []) {
        if ($model->validate()) {
            if (empty($systemKey)) {
                $systemKey = self::$defaultKey;
            }
    ...
    public function beforeSave($insert)
    {
        ...
        if (parent::beforeSave($insert)) {
            ...
            if ($insert) {
                ...
                $this->prepareData($this, $currentDate, $currentUserId, $this->systemKey, false, self::$saveUpdateVersion, $this->checkKeyExist, $this->needReturnKey, $errMsg);
            }
...
//                    self::unsetUnNeedInfo($this);
                    if ($depandMode == self::DEPEND_MODE_SQL) {
                        ...
                        self::saveToCache($cacheKey, $this, $dependencySql, $duration, $withCache, true);
                    } else {
                        self::saveToCache($cacheKey, $this, null, $duration, $withCache, true);
                    }
                }
            }
        }

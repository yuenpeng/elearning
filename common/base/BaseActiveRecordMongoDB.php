...
//    const DEPEND_MODE_SQL = "sql";
    ...
    private static function getForcePhysicalDeleteCollection() {
        //部分表不存在删除操作,可以强制物理删除
        $forcePhysicalDeleteCollection = [
            'eln_ln_scorm_scoes_track'
        ];

        return $forcePhysicalDeleteCollection;
    }

    private static function getForceCacheCollection() {
        //需要强制缓存的表名，建议只放数量少或不频繁更新的表
        $forceCacheCollection = [
            'eln_ln_scorm_scoes_track'=>['depend'=>self::DEPEND_MODE_DURATION,'duration'=>self::DURATION_MONTH]
        ];

        return $forceCacheCollection;
    }
    ...
    {
        $collectionName = self::getFullName();
        $forcePhysicalDeleteCollection = self::getForcePhysicalDeleteCollection();
        if (!array_key_exists($collectionName, $forcePhysicalDeleteCollection)) {
            if (!$includeDeleted) {
...
//        $duration = 0;
        $collectionName = self::getFullName();
        $forceCacheCollection = self::getForceCacheCollection();
        if (count($forceCacheCollection) > 0) {
            if (array_key_exists($collectionName, $forceCacheCollection)) {
                $depandMode = $forceCacheCollection[$collectionName]["depend"];
                $duration = $forceCacheCollection[$collectionName]["duration"];
                $withCache = true;
            }
        }
        $forcePhysicalDeleteCollection = self::getForcePhysicalDeleteCollection();
    ...
    public static function getFullName(){
        $collectionName = self::getCollection()->getFullName();
    ...
    public function beforeSave($insert)
    {
        ...
        if (parent::beforeSave($insert)) {
            ...
            if ($insert) {
                ...
                $this->prepareData($this, $currentDate, $currentUserId, $this->systemKey, false, self::$saveUpdateVersion, $errMsg);
            }

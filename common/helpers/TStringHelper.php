...
class TStringHelper
{
    ...
    public static function Theme($course, $is_big = false)
    {
        $default = '/static/frontend/images/course_theme_big.png';// : '/static/frontend/images/course_theme_small.png';

        $result = '';

        $fileModelHelper = new TFileModelHelper();

        if ($is_big) {
            $result = empty($course['theme_url']) ? $default : $fileModelHelper->secureLink($course['theme_url']);
        } else {
            $result = empty($course['theme_url']) ? $default : $fileModelHelper->secureLink($course['theme_url'] . '_378x225.jpg');
        }
    ...
    public static function getCourseTime(LnCourse $course)
    {
        ...
        if ($course->course_type === LnCourse::COURSE_TYPE_ONLINE) {
            if (empty($course->end_time)) {
                $text = Yii::t('frontend', 'forever');
            } else {
                if (date('Y', $course->start_time) === date('Y', $course->end_time)) {
                    $text = date(Yii::t('common', 'date_format_ymd'), $course->start_time) . '-' . date(Yii::t('common', 'date_format_md'), $course->end_time);
                } else {
                    $text = date(Yii::t('common', 'date_format_ymd'), $course->start_time) . '-' . date(Yii::t('common', 'date_format_ymd'), $course->end_time);
                }
            }
        } elseif ($course->course_type === LnCourse::COURSE_TYPE_FACETOFACE) {
            if (date('Y', $course->open_start_time) === date('Y', $course->open_end_time)) {
                $text = date(Yii::t('common', 'date_format_ymd'), $course->open_start_time) . '-' . date(Yii::t('common', 'date_format_md'), $course->open_end_time);
            } else {
                $text = date(Yii::t('common', 'date_format_ymd'), $course->open_start_time) . '-' . date(Yii::t('common', 'date_format_ymd'), $course->open_end_time);
            }
        }
        return $text;
    }
    ...
    */
     ...
     * @return string
     */
    public static function genQRCode($string, $type = 'course', $courseId = ''){
        if ($type == 'course'){
            $url = \Yii::$app->urlManager->getHostInfo();
            if($courseId){
                $text = $url . \Yii::$app->urlManager->createUrl(['resource/course/scan-view', 'code' => $string, 'course_id'=>$courseId]);
            }else{
     ...
     * @return string|void
     */
    public static function getThemeSecretLink($theme){
        if (empty($theme)) return ;
        $tfile = new TFileModelHelper();
        return $tfile->secureLink($theme);
    }
     ...
     * @return array|mixed
     */
    public static function getRadioYesAndNo($val = null){
        $array = [
            BaseActiveRecord::NO => Yii::t('common', 'no'),
            BaseActiveRecord::YES => Yii::t('common', 'yes'),
        ];
        if (is_null($val)){
            return $array;
        }else{
     ...
     * @return int
     */
    public static function  CheckAccountType($account = null){
        if(preg_match("/^[0|1]\d{7,8}$/", $account)){
            $type = '1';//工号
        }elseif (preg_match("/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|[xX])$/", $account)) {//校验规则：6位地址编码+8位出生日期+3位顺序号+1位校验位
            $type = '2';//身份证（第二代） 
        }elseif (preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/", $account)) {
            $type = '3';//邮箱
        }elseif(preg_match("/^1[3|4|5|8][0-9]\d{4,8}$/", $account)) {
            $type = '4';//手机号码
        }else{ //未知-默认工号
            $type = '1';
        }

...
class TTimeHelper
{
     ...
     * @param null $course_type 课程类型
     * @return string ''、'快过期'、'已过期'|'已结课'
     */
    public static function getCourseExpiredTag($end_time, $course_type = null)
    {
        $current = time();
        ...
        if ($end_time < $current) {
            if ($course_type === LnCourse::COURSE_TYPE_FACETOFACE) {
                return Yii::t('frontend', 'course_end');
            } else {

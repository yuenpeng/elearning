...
class TFileModelHelper{
     ...
     * @return string
     */
    public static function secureLink($filePath){
     ...
     * @return string
     */
    public static function convertSecureLink($url)
    {
        if (empty($url)) {
            return "";
        } else {
            return self::secureLink($url);
        }
    }
...
//                    header("Content-Disposition: attachment; filename=\"". urlencode($downloadFileName)."\";" ); header("Content-Transfer-Encoding: binary");//urlencode是解决乱码问题
    ...
    public function isScormDir($scormPath){
        ...
        if(file_exists($filePath)) {
            ...
            if (!$isUTF8) {
                $encode = mb_detect_encoding($originContent, array('UTF-8','GBK','LATIN1','BIG5', 'Unicode'));
                if ($encode == 'ISO-8859-1') {
//                    $originContent = TStringHelper::unicodeToUtf8($originContent);
                }else{
...
//                            break;
    ...
    public function UploadFile($file, $uploadBatch = null, $transferType = LnComponent::TRANSFER_TYPE_NORMAL, $fileTypes = null, $allowSameFile = false){
        ...
        if (!$skipSaveFile) {
            ...
            $HomeworkExtractRelativePath = $this->HomeworkExtractPath.substr($hashfilename,0,2).'/';
            ...
            $ScormExtractRelativePath = $this->ScormExtractPath.substr($hashfilename,0,2).'/';
            ...
            $AiccExtractRelativePath = $this->AiccExtractPath.substr($hashfilename,0,2).'/';
            $HomewrokExtractRelativePath = $this->HomeworkExtractPath.substr($hashfilename,0,2).'/';
            $AudioOriginRelativePath = $this->AudioOriginPath.substr($hashfilename,0,2).'/';
            $AudioBackupRelativePath = $this->AudioBackupFilePath.substr($hashfilename,0,2).'/';
            $VideoOriginRelativePath = $this->VideoOriginPath.substr($hashfilename,0,2).'/';
            $VideoBackupRelativePath = $this->VideoBackupFilePath.substr($hashfilename,0,2).'/';
            ...
            $ScormExtractAbsolutePath = $this->ScormExtractAbsolutePath.substr($hashfilename,0,2).'/';
            ...
            $AiccExtractAbsolutePath = $this->AiccExtractAbsolutePath.substr($hashfilename,0,2).'/';
            $HomeworkExtractAbsolutePath = $this->HomeworkExtractAbsolutePath.substr($hashfilename,0,2).'/';
            $AiccBackupAbsolutePath = $this->AiccBackupFilePhysicalPath.substr($hashfilename,0,2).'/';
            $AudioOriginAbsolutePath = $this->AudioOriginPhysicalPath.substr($hashfilename,0,2).'/';
            $AudioBackupAbsolutePath = $this->AudioBackupFilePhysicalPath.substr($hashfilename,0,2).'/';
            $VideoOriginAbsolutePath = $this->VideoOriginPhysicalPath.substr($hashfilename,0,2).'/';
            $VideoBackupAbsolutePath = $this->VideoBackupFilePhysicalPath.substr($hashfilename,0,2).'/';
            ...
            $uploadAudioOriginFilePath  = $AudioOriginRelativePath . $hashfilename . '.' . $fileExtension;
            $uploadAudioBackupFilePath  = $AudioBackupRelativePath . $hashfilename . '.' . $fileExtension;
            $uploadVideoOriginFilePath  = $VideoOriginRelativePath . $hashfilename . '.' . $fileExtension;
            $uploadVideoBackupFilePath  = $VideoBackupRelativePath . $hashfilename . '.' . $fileExtension;
...
//            $uploadExtractAbsolutePath  = Yii::$app->basePath.'/..' . $ExtractRelativePath . $hashfilename . '/'; //上传解压文件所在目录绝对路径
            ...
            $uploadScormExtractAbsolutePath = rtrim(Yii::getAlias($ScormExtractAbsolutePath), '/\\') ."/" . $hashfilename ."/";//上传解压文件所在目录绝对路径
            ...
            $uploadAiccExtractAbsolutePath = rtrim(Yii::getAlias($AiccExtractAbsolutePath), '/\\') ."/" . $hashfilename ."/";//上传解压文件所在目录绝对路径
            $uploadHomeworkExtractAbsolutePath = rtrim(Yii::getAlias($HomeworkExtractAbsolutePath), '/\\') ."/" . $hashfilename ."/";//上传解压文件所在目录绝对路径
            $uploadVideoOriginAbsolutePath = rtrim(Yii::getAlias($VideoOriginAbsolutePath), '/\\') . "/";//Video上传原文件所在目录绝对路径
            $uploadVideoBackupAbsolutePath = rtrim(Yii::getAlias($VideoBackupAbsolutePath), '/\\') . "/";//Video上传备份文件所在目录绝对路径
            $uploadAudioOriginAbsolutePath = rtrim(Yii::getAlias($AudioOriginAbsolutePath), '/\\') . "/";//Audio上传原文件所在目录绝对路径
            $uploadAudioBackupAbsolutePath = rtrim(Yii::getAlias($AudioBackupAbsolutePath), '/\\') . "/";//Audio上传备份文件所在目录绝对路径

            //检查并创建文件夹
            if (stripos($fileType, 'audio') !== false){
                //音频
                TFileHelper::check_exist_dir($uploadAudioOriginAbsolutePath);
                TFileHelper::check_exist_dir($uploadAudioBackupAbsolutePath);
                $uploadOriginFilePath = $uploadAudioOriginFilePath;
                $uploadBackupFilePath = $uploadAudioBackupFilePath;
                $targetOrginFile = $uploadAudioOriginAbsolutePath . $hashfilename .'.' . $fileExtension;
                $targetBackupFile = $uploadAudioBackupAbsolutePath . $hashfilename . '.' .$fileExtension;
            }elseif (stripos($fileType, 'video') !== false){
                //视频
                TFileHelper::check_exist_dir($uploadVideoOriginAbsolutePath);
                TFileHelper::check_exist_dir($uploadVideoBackupAbsolutePath);
                $uploadOriginFilePath = $uploadVideoOriginFilePath;
                $uploadBackupFilePath = $uploadVideoBackupFilePath;
                $targetOrginFile = $uploadVideoOriginAbsolutePath . $hashfilename .'.' . $fileExtension;
                $targetBackupFile = $uploadVideoBackupAbsolutePath . $hashfilename . '.' .$fileExtension;
            }else{
        ...
        if (!$skipSaveFile) {
            if ($fileExtension == "zip")//如果是zip文件，先做解压缩
            {
                ...
                if ($manifestInfo != false) {
                    TFileHelper::unzip($targetOrginFile, $uploadScormExtractAbsolutePath);
                    TFileHelper::removeDir(rtrim(Yii::getAlias($ExtractAbsolutePath), '/\\') ."/");
                    ...
                    if ($manifestInfo != false) {
                        TFileHelper::unzip($targetOrginFile, $uploadAiccExtractAbsolutePath);
                        TFileHelper::removeDir(rtrim(Yii::getAlias($ExtractAbsolutePath), '/\\') ."/");
                        ...
                        if ($entranceAddress != false) {
                            ...
                            TFileHelper::unzip($targetOrginFile, $uploadHomeworkExtractAbsolutePath);
                            TFileHelper::removeDir(rtrim(Yii::getAlias($ExtractAbsolutePath), '/\\') ."/");
                        }
                    }
                }
        ...
        if (!isset($fileModel) || $fileModel == null) {
            ...
            if ($isScorm)
            {
                $fileDir = $ScormExtractRelativePath . $hashfilename . '/';
            ...
            {
                $fileDir = $AiccExtractRelativePath . $hashfilename . '/';
            ...
            {
                $fileDir = $HomeworkExtractRelativePath . $hashfilename . '/';

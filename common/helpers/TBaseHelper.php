...
class TBaseHelper
{
    ...
    protected static $supported_lang = ['zh-CN', 'en-US', 'zh-TW', 'ko-KR', 'ja-JP'];
    ...
    public static function getSupportedLang($index = null)
    {
        return !is_null($index) && is_integer($index) ?
            (isset(self::$supported_lang[$index]) ? self::$supported_lang[$index] : self::$supported_lang[0]) :
            self::$supported_lang;
    }
    ...
    public static function getLanguage()
    {
        try {
            ...
            if (!empty(Yii::$app->request->getQueryParam("lang"))) {
                ...
                $language = in_array($language, self::$supported_lang) ? $language : self::$supported_lang[0];
            }
...
//            $macAddress = $osType;
    ...
    {
        return true; //BOEU 购买源代码之后，去掉License验证
